...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        /// Gets an order
        /// </summary>
        /// <param name="customerId">The custom order number</param>
        /// <returns>Order</returns>
        public virtual List<Order> GetOrderByCustomOrderById(int customerId)
        {
            if (customerId == 0)
                return null;

            var response = _orderRepository.Table.Where(o => o.CustomerId == customerId);
            if(response != null)
            {
                response = response.Where(o => o.Deleted == false && (o.OrderStatusId == (int)OrderStatus.Pending || o.OrderStatusId == (int)OrderStatus.Processing));
            }
        ...
        public virtual IPagedList<Order> SearchOrders(int storeId = 0,
            ...
            string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false,bool deleted=false)
        {
            ...
            query = deleted?query.Where(o => o.Deleted): query.Where(o => !o.Deleted);
            query = query.OrderByDescending(o => o.CreatedOnUtc);

            //database layer paging
            return new PagedList<Order>(query, pageIndex, pageSize, getOnlyTotalCount);
        }
        public int OrderCountforproduct(int customerId = 0, int productId = 0)
        {
        ...
        queryOrder
               .Where(o => o.Id == oi.OrderId && o.OrderStatusId == (int)OrderStatus.Complete)
        ...
        /// <param name="getOnlyTotalCount">A value in indicating whether you want to load only total number of records. Set to "true" if you don't want to load data from database</param>
        /// <returns>Orders</returns>
        public virtual IPagedList<Order> SearchOrdersforReview(int storeId = 0,
            int vendorId = 0, int customerId = 0,
            int productId = 0, int affiliateId = 0, int warehouseId = 0,
            int billingCountryId = 0, string paymentMethodSystemName = null,
            DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            ...
            string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, bool deleted = false)
        {
            ...
            //            join oi in _orderItemRepository.Table on o.Id equals oi.OrderId
            //            join p in _productRepository.Table on oi.ProductId equals p.Id
            //            where p.VendorId == vendorId && oi.Isviewed == false
            //            select o;

            if (customerId > 0)
            {
                query = query.Where(o => o.CustomerId == customerId  && o.Deleted == false && o.OrderStatusId == (int)OrderStatus.Complete);
            }
            //if (productId > 0)
            //    query = from o in query
            //            join oi in _orderItemRepository.Table on o.Id equals oi.OrderId
            //            where oi.ProductId == productId && oi.Isviewed == false
            //            select o;
        ...
        public virtual List<Order> GetNotExcutedOrders()
        {
            var query = _orderRepository.Table;

            List<int> osId = new List<int>();
            var StratDate = DateTime.Now.AddDays(-2);

            osId.Add((int)OrderStatus.Pending);
            osId.Add((int)OrderStatus.Processing);

            query = query.Where(o => StratDate >= o.CreatedOnUtc
                                         && osId.Contains(o.OrderStatusId)
                                                  && !o.Deleted);
            return query.ToList();
        }
        #endregion
        ...
        public virtual IList<OrderItem> GetOrderItems(int orderId, bool? isNotReturnable = null, bool? isShipEnabled = null, int vendorId = 0, int productId=0)
        {
            if (orderId == 0)
        ...
        public virtual void UpdateOrderItemByProductIdAnCustomerId(int productId, int customerId, int orderId = 0)
        {
            if (customerId == 0)
            ...
            //    .Where(o => o.CustomerId == customerId && o.OrderStatusId == (int)OrderStatus.Complete && !o.Deleted)
            //    .ToList();

            //var query = _orderRepository.Table;

            //query = query.Where(p => !p.Deleted && p.CustomerId == customerId && p.OrderStatusId == (int)OrderStatus.Complete && p.Id == orderId);

            //var OrderItemquery = from orderItem in query
            //                     join orderItem in _orderItemRepository.Table on order.Id equals orderItem.OrderId

            //                     where 
            //                         orderItem.Isviewed == false && orderItem.ProductId == productId && orderItem
            //                     select orderItem;

            var query = _orderItemRepository.Table.FirstOrDefault(x => x.OrderId == orderId && x.ProductId == productId);

            if (query != null)
            {
                query.Isviewed = true;
                _orderItemRepository.Update(query);
                //event notification
                _eventPublisher.EntityUpdated(query);
            }
        ...
        public bool IsDisplayed(int productId, int customerId, int orderId)
        {
            var query = _orderItemRepository.Table.FirstOrDefault(x => x.OrderId == orderId && x.ProductId == productId);
            return query!=null?query.Isviewed: false;
        }
