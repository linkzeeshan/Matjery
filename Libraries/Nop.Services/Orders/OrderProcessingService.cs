...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <param name="order">Order</param>
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order, IEnumerable<ShoppingCartItem> gCart)
        {
            decimal totalPerOrder = 0;

            foreach (ShoppingCartItem cart in gCart)
            {
                ShoppingCartItem sc = _shoppingCartService.GetShoppingCartItemById(cart.Id);
                ...
                order.OrderSubtotalInclTax = scSubTotalInclTax;
                order.OrderSubtotalExclTax = scSubTotalExclTax;
                order.OrderSubTotalDiscountExclTax = discountAmountExclTax;
                order.OrderSubTotalDiscountInclTax = discountAmountInclTax;
                ...
                var orderTotal = _orderTotalCalculationService.GetShoppingCartTotal(new List<ShoppingCartItem> { sc }, out orderDiscountAmount,
                    out orderAppliedDiscounts, out appliedGiftCards, out redeemedRewardPoints, out redeemedRewardPointsAmount);
                if (!orderTotal.HasValue)
                    throw new NopException("Order total couldn't be calculated");
                totalPerOrder += orderTotal.Value;
                //order.OrderTotal += orderTotal.Value;
                order.OrderTotal = totalPerOrder;
                _orderService.UpdateOrder(order);
                //attributes
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    List<ShoppingCartItem> carts = details.Cart.ToList();
                    //move shopping cart items to order items
                    var groupedCart = carts.GroupBy(c => _productService.GetProductById(c.ProductId).VendorId);
                    foreach (var gCart in groupedCart)
                    {

                        var order = SaveOrderDetails(processPaymentRequest, processPaymentResult, details);
                        // result.PlacedOrder = order; commented as per customizationx

                        //move shopping cart items to order items
                        MoveShoppingCartItemsToOrderItems(details, order, gCart);
                        orders.Add(order);

                    }
                    details.Cart.ToList().ForEach(sci => _shoppingCartService.DeleteShoppingCartItem(sci, false));
                    result.PlaceOrders = orders;
                    foreach (var order in orders)
                    {
                        foreach (var odreritem in order.OrderItems)
                        {
                            _orderService.InsertOrderItem(odreritem);
                        }
        ...
        public virtual void Deliver(Shipment shipment, bool notifyCustomer)
        {
            ...
            //push notification
            _queuedPushNotificationService.QueueOrderCompletedCustomerNotification(order);
            //add a note
        ...
        /// <param name="order">Order</param>
        /// <param name="notifyCustomer">True to notify customer</param>
        public virtual void CancelOrder(Order order, bool notifyCustomer, string Reason = "")
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            //add a note
            AddOrderNote(order, string.IsNullOrEmpty(Reason)?"Order has been cancelled":"Cancelled: " +Reason);
