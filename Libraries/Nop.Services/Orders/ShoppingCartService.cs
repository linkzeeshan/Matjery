...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        public virtual ShoppingCartItem GetShoppingCartItemById(int id)
        {
                ...
                switch (product.ManageInventoryMethod)
                {
                    ...
                    case ManageInventoryMethod.ManageStock:
                        if (product.BackorderMode == BackorderMode.NoBackorders)
                        {
                            ...
                            if (maximumQuantityCanBeAdded < quantity && shoppingCartType== ShoppingCartType.ShoppingCart)
                            {
            ...
            foreach (var a2 in attributes2)
            {
                if (a2.IsRequired)
                {
                    ...
                    if (!found)
                    {
                        ...
                        if (!string.IsNullOrEmpty(_localizationService.GetLocalized(productAttribute,a => a.TextPrompt)))
                            warnings.Add(_localizationService.GetLocalized(productAttribute, a => a.TextPrompt));
                        else
                            warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.SelectAttribute"), _localizationService.GetLocalized(productAttribute,a => a.Name)));
                    }
                }
            ...
            int quantity = 1, bool addRequiredProducts = true)
        {
            ...
            if (shoppingCartItem != null)
            {
                if (shoppingCartItem.Quantity == 1 && quantity < 0)
                {
                ...
                {
                    //update existing shopping cart itemd
