...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, bool? overridePublished = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(categoryName))
                query = from q in query
                        join lpj in (from lp in _localizedPropertyRepository.Table where lp.LocaleKeyGroup == "Category" && lp.LocaleKey == "Name" select lp)
                        on q.Id equals lpj.EntityId
                         into join1
                        from j in join1.DefaultIfEmpty()
                        where j.LocaleValue.Contains(categoryName) || q.Name.Contains(categoryName)
                        select q;

           

           //query = query.Where(c => c.Name.Contains(categoryName)); COMMENTED AS PER CUSTOMIZATION
        ...
        public virtual IPagedList<Category> GetAllCategories(int languageId, string categoryName = "", int storeId = 0,
            ...
            string separator = ">>", int languageId = 0)
        {
            ...
            {
                var categoryName = _localizationService.GetLocalized(breadcrumb[i], x => x.Name, _workContext.WorkingLanguage.Id);
        ...
        public virtual Category GetCategoryByOldSystemId(int categoryId)
        {
            if (categoryId == 0)
                return null;

            return _categoryRepository.Table.FirstOrDefault(c => c.OldSystemCategoryId == categoryId);
        }
