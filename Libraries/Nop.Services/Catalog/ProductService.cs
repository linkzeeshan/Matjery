...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        private readonly IRepository<CustomerTranslationMapping> _customertranslatorRepository;
        ...
        public ProductService(CatalogSettings catalogSettings,
            ...
            IRepository<CustomerTranslationMapping> customertranslatorRepository,
            ...
            
        {
            ...
            _customertranslatorRepository = customertranslatorRepository;
        ...
        public virtual IPagedList<Product> GetAllProductsDisplayedOnHomePage(int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = from p in _productRepository.Table
                        orderby p.DisplayOrder, p.Name
                        where p.Published &&
                        !p.Deleted && p.ShowOnHomepage
                        select p;

            var products = new PagedList<Product>(query, pageIndex, pageSize);
            return products;
        }
        public virtual IPagedList<Product> GetAllProductsDiscounted(int pageIndex, int pageSize = int.MaxValue)
        {
            var query = from p in _productRepository.Table
                        join v in _vendorRepository.Table
                        on p.VendorId equals v.Id
                        where v.Active == true
                        orderby p.UpdatedOnUtc descending, p.DisplayOrder ascending, p.Name ascending
                        where p.Published && !p.Deleted && p.OldPrice > 0 
                        select p;
            var products = new PagedList<Product>(query, pageIndex, pageSize);
            return products;
        }
        public virtual IPagedList<Product> GetRecentlyAddedProducts(int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = from p in _productRepository.Table
                        where p.Published && !p.Deleted && p.VendorId > 0 && p.ProductPictures.Count > 0
                        select p;

            var groupedProducts = from p in query
                                  group p by p.VendorId
                                   into pGroup
                                  orderby pGroup.Key
                                  select pGroup.FirstOrDefault();
            groupedProducts = groupedProducts.OrderByDescending(p => p.CreatedOnUtc);
            var products = new PagedList<Product>(groupedProducts, pageIndex, pageSize);
            return products;
        }
        public virtual Product GetProductByOldSystemId(int productId)
        {
            if (productId == 0)
                return null;

            return _productRepository.Table.FirstOrDefault(p => p.OldSystemProductId == productId);
        }
        public virtual IList<Product> GetProductsByVendorId(int id)
        {
        ...
        public virtual int GetNumberOfProductsInCategory(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            //fillter by active vendor
            query = from p in query
                    join v in _vendorRepository.Table
                    on p.VendorId equals v.Id
                    where p.VendorId == v.Id && v.Active == true
                    select p;
            ...
            if (_customerService.IsTranslator(_workContext.CurrentCustomer))
            {
                // is transport
                query = from p in query
                        join ct in _customertranslatorRepository.Table
                        on new { c1 = p.Id, c2 = "Product" } equals new { c1 = ct.EntityId, c2 = ct.EntityName } into p_acl
                        from acl in p_acl.DefaultIfEmpty()
                        select p;
            }
            ...
            bool? overridePublished = null)
        {
            ...
            var products = _productRepository.EntityFromSql(_customerService.IsAdmin(_workContext.CurrentCustomer)? "AdminProductLoadAllPaged" : "ProductLoadAllPaged",
                pCategoryIds,
                pManufacturerId,
                pStoreId,
                pVendorId,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            var combinations = from pac in _productAttributeCombinationRepository.Table
                ...
                where
                   ...
                   // p.ManageInventoryMethodId == (int)ManageInventoryMethod.ManageStockByAttributes &&  //COMMENTED AS PER CUSTOMIZATION
                    //ignore deleted products
                    !p.Deleted &&
                    //ignore grouped products
        ...
        public IPagedList<Product> deletedProduct(int pageIndex = 0,int pageSize = int.MaxValue,int vendorid=0)
        {
            ...
            if (vendorid > 0)
            {
                query = query.Where(p => p.Deleted && p.VendorId==vendorid).OrderByDescending(d => d.UpdatedOnUtc);
            }
            else
            {
                query = query.Where(p => p.Deleted).OrderByDescending(d => d.UpdatedOnUtc);
            }
            var products = new PagedList<Product>(query, pageIndex, pageSize);
        ...
        //public virtual IPagedList<ProductReview> IsReviewDone(int customerId = 0, int productId = 0, int[] orderIDs = null)
        //{
        //    var query = _orderItemRepository.Table;
        //    query = query.Where(pr => pr.Isviewed == false);

        //    if (customerId > 0)
        //        query = query.Where(pr => pr.CustomerId == customerId);

        //    query = query.Where(pr => pr.order.Contains(message));
        ...
        public virtual IPagedList<Product> SearchProductsMod(
            out IList<int> filterableSpecificationAttributeOptionIds,
            bool loadFilterableSpecificationAttributeOptionIds = false,
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            ...
            int ExcludeInactiveVendor = 0)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                var pVendorId = SqlParameterHelper.GetInt32Parameter("VendorId", vendorId);
                var pWarehouseId = SqlParameterHelper.GetInt32Parameter("WarehouseId", warehouseId);
                var pProductTypeId = SqlParameterHelper.GetInt32Parameter("ProductTypeId", productType.HasValue ? (int)productType.Value :0);
                var pVisibleIndividuallyOnly = SqlParameterHelper.GetBooleanParameter("VisibleIndividuallyOnly", visibleIndividuallyOnly);
                var pMarkedAsNewOnly = SqlParameterHelper.GetBooleanParameter("MarkedAsNewOnly", markedAsNewOnly);
                var pProductTagId = SqlParameterHelper.GetInt32Parameter("ProductTagId", productTagId);
                var pFeaturedProducts = SqlParameterHelper.GetBooleanParameter("FeaturedProducts", featuredProducts);
                ...
                var pOverridePublished = SqlParameterHelper.GetBooleanParameter("OverridePublished", overridePublished);
                var pLoadFilterableSpecificationAttributeOptionIds = SqlParameterHelper.GetBooleanParameter("LoadFilterableSpecificationAttributeOptionIds", loadFilterableSpecificationAttributeOptionIds);
                var pFilterableSpecificationAttributeOptionIds = SqlParameterHelper.GetStringParameter("FilterableSpecificationAttributeOptionIds", (int.MaxValue - 1).ToString());
                var pTotalRecords = SqlParameterHelper.GetInt32Parameter("TotalRecords",(int)ParameterDirection.Output);

                //invoke stored procedure
                var products = _dataProvider.ExecuteStoredProcedure<IEnumerable<Product>>(
                    "ProductLoadAllPaged",
                    pCategoryIds,
                    pManufacturerId,
                    pStoreId,
                ...
                return new PagedList<Product>(products, pageIndex, pageSize, totalRecords);
                //PagedList(IEnumerable<T> source, int pageIndex, int pageSize, int totalCount)
        ...
        public IPagedList<Product> SearchProductsMod(int pageIndex = 0, int pageSize = int.MaxValue, IList<int> categoryIds = null, int manufacturerId = 0, int storeId = 0, int vendorId = 0, int warehouseId = 0, ProductType? productType = null, bool visibleIndividuallyOnly = false, bool markedAsNewOnly = false, bool? featuredProducts = null, decimal? priceMin = null, decimal? priceMax = null, int productTagId = 0, string keywords = null, bool searchDescriptions = false, bool searchManufacturerPartNumber = true, bool searchSku = true, bool searchProductTags = false, int languageId = 0, IList<int> filteredSpecs = null, ProductSortingEnum orderBy = ProductSortingEnum.Position, bool showHidden = false, bool? overridePublished = null, int ExcludeInactiveVendor = 0)
        {
            IList<int> filterableSpecificationAttributeOptionIds;

            return SearchProductsMod(out filterableSpecificationAttributeOptionIds, false,
                pageIndex, pageSize, categoryIds, manufacturerId,
                storeId, vendorId, warehouseId,
                productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts,
                priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku,
                searchProductTags, languageId, filteredSpecs,
                orderBy, showHidden, overridePublished, ExcludeInactiveVendor);
        }

        public IPagedList<OrderItem> GetCustomerLastProductOrderItem(int customerId = 0, bool? approved = null, DateTime? fromUtc = null, DateTime? toUtc = null, int storeId = 0, int productId = 0, int vendorId = 0, bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue, int top = 3)
        {
            var query = _orderRepository.Table;
            query = query.Where(p => p.Deleted == false && p.CustomerId == customerId && p.OrderStatusId == (int)OrderStatus.Complete);
            var OrderItemquery = from order in query
                                 join orderItem in _orderItemRepository.Table on order.Id equals orderItem.OrderId
                                 where orderItem.Isviewed == false
                                 select orderItem;

            var productQuery = from orderItem in OrderItemquery
                               join product in _productRepository.Table on orderItem.ProductId equals product.Id
                               where !product.Deleted && product.Published && product.VisibleIndividually
                               select orderItem;

            productQuery = productQuery.OrderByDescending(pr => pr.ProductId).Distinct();

            var products = new PagedList<OrderItem>(productQuery, pageIndex, pageSize);
            return products;
        }

        public IPagedList<Product> GetCustomerTopProductOrderItem(int customerId = 0, bool? approved = null, DateTime? fromUtc = null, DateTime? toUtc = null, int storeId = 0, int productId = 0, int vendorId = 0, bool showHidden = false, int pageIndex = 0, int pageSize = int.MaxValue, int top = 3)
        {
            var query = _orderRepository.Table;
           // var query = _productRepository.Table;
           // query = query.Where(p => !p.Deleted && p.Published && p.VisibleIndividually);
            query = query.Where(p => p.Deleted== false && p.CustomerId == customerId && p.OrderStatusId == (int)OrderStatus.Complete);
             ...
             var OrderItemquery = from order in query
                    ...
                    where
                        ...
                        //(order.CustomerId == customerId && order.Deleted == false && order.OrderStatusId == (int)OrderStatus.Complete) && 
                        orderItem.Isviewed == false
                    select orderItem;
            ...
            var productQuery = from orderItem in OrderItemquery
                               ...
                               !product.Deleted && product.Published && product.VisibleIndividually

                               select product;


            productQuery = productQuery.OrderBy(pr => pr.CreatedOnUtc).ThenBy(pr => pr.Id);
            productQuery = productQuery.OrderByDescending(pr => pr.CreatedOnUtc).ThenBy(pr => pr.Id).Distinct();

            var products = new PagedList<Product>(productQuery, pageIndex, pageSize);
            return products;
        }
        /// <summary>
        /// Gets a product by SKU
        ...
        /// <returns>Product</returns>
        public Product GetProductByName(string name, int vendorId = 0)
        {
            ...
            if (vendorId != 0)
            {
                 var queryVendor = from p in _productRepository.Table
                            orderby p.Id
                            where !p.Deleted &&
                            p.Name == name && p.VendorId == vendorId
                            select p;
                 var vendorproduct = queryVendor.FirstOrDefault();
