...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
      ...
      int pageIndex = 0, int pageSize = int.MaxValue, CustomerSortingEnum OrderBy = CustomerSortingEnum.Position,
      int countryId = 0, int stateProvinceId = 0)
        {
           ...
           if (!String.IsNullOrWhiteSpace(email))
            ...
            if (countryId > 0)
            {
                var newCountryId = Convert.ToString(countryId);
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomerDefaults.NationalityCountryId &&
                        z.Attribute.Value.Contains(newCountryId)))
                    .Select(z => z.Customer);
            }
            if (stateProvinceId > 0)
            {
                var newStateProvinceId = Convert.ToString(stateProvinceId);
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomerDefaults.StateProvinceIdAttribute &&
                        z.Attribute.Value.Contains(newStateProvinceId)))
                    .Select(z => z.Customer);
            }
            ...
            //if (OrderBy != CustomerSortingEnum.Position)
            //{
            //    if (OrderBy == CustomerSortingEnum.CreatedOnDesc)
            //    {
            ...
            //        switch (OrderBy)
            //        {
            //            //case CustomerSortingEnum.EmailAsc: query = query.OrderBy(c => c.Email); break;
            //            //case CustomerSortingEnum.EmailDesc: query = query.OrderByDescending(c => c.Email); break;

            //            //case CustomerSortingEnum.UserNameAsc: query = query.OrderBy(c => c.Username); break;
            //            //case CustomerSortingEnum.UserNameDesc: query = query.OrderByDescending(c => c.Username); break;

            //            ////case CustomerSortingEnum.NameAsc: query = query.OrderBy(c => c.GetFullName()); break;
            //            ////case CustomerSortingEnum.NameDesc: query = query.OrderByDescending(c => c.GetFullName()); break;

            //            //case CustomerSortingEnum.CreatedOnAsc: query = query.OrderBy(c => c.CreatedOnUtc); break;
            //            //case CustomerSortingEnum.CreatedOnDesc: query = query.OrderByDescending(c => c.CreatedOnUtc); break;
            //        }
            //    }
        ...
        public virtual void DeleteCustomer(Customer customer)
        {
            ...
            //change email format
            customer.Email = CommonHelper.CustomerEmailFormat(customer.Email, true);
            //change user name format
            customer.Username = CommonHelper.CustomerNameFormat(customer.Username, true);
        ...
        /// Gets a customer
        /// </summary>
        /// <param name="vendorId">Customer identifier</param>
        /// <returns>A customer</returns>
        ...
        public virtual Customer GetCustomerByOldSystemCustomerId(int oldSystemId)
        {
            if (oldSystemId == 0)
                return null;

            return _customerRepository.Table.FirstOrDefault(c => c.CustomerOldSystemId == oldSystemId);
        }
        ...
        public virtual Customer GetCustomerByEmail(string email, bool checkForDelete = false)
        ...
        /// <returns>Customer</returns>
        public virtual Customer GetCustomerByUsername(string username, bool checkFordelete = false)
        {
            if (string.IsNullOrWhiteSpace(username))
                return null;
        ...
        public virtual bool IsFoundation( Customer customer, bool onlyActiveCustomerRoles = true)
        {
            return IsInCustomerRole(customer, NopCustomerDefaults.Foundations, onlyActiveCustomerRoles);
        }
        public virtual bool IsTranslator( Customer customer, bool onlyActiveCustomerRoles = true)
        {
        ...
        /// <summary>
        /// Gets deleted customers
        /// </summary>
        /// <returns>Result</returns>
        public IPagedList<Customer> Deletedcustomers(int pageIndex = 0,
         int pageSize = int.MaxValue)
        {

            #region Search products
            //products
            var queryTable = _customerRepository.Table;
           var query = queryTable.Where(p => p.Deleted).OrderByDescending(d => d.LastActivityDateUtc);
            //query = query.Where(d => !CompareDeleteCustomerRegistrationExist(d, query));
           // List<Customer> customerslist = new List<Customer>();
           // var NotDeleted = queryTable.Where(p => !p.Deleted).OrderByDescending(d => d.LastActivityDateUtc);

           // customerslist = query.Where(q => !CompareDeleteCustomerRegistrationExist(q, NotDeleted)).ToList();
        ...
        public bool CompareDeleteCustomerRegistrationExist(Customer customer, IQueryable<Customer> query)
        {
            if (customer.Email.Contains("deleted_"))
            {
                var changeEmailFormat = customer.Email.Replace("deleted_", "");
                var deleteCustomerExist = query.FirstOrDefault(q => q.Email == changeEmailFormat && !q.Deleted);
