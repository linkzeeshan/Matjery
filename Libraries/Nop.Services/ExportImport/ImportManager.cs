...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ImportManager : IImportManager
    {
        ...
        public ImportManager(CatalogSettings catalogSettings,
            ...
            ICustomerRegistrationService customerRegisterationService,
            ...
            )
        {
            ...
            _customerRegistrationService = customerRegisterationService;
        ...
        private static void SetOutLineForSpecificationAttributeRow(object cellValue, ExcelWorksheet worksheet, int endRow)
        {
            ...
            {
                if (SpecificationAttributeType.Option.ToSelectListInServices(useLocalization: false)
        ...
        private ImportProductMetadata PrepareImportProductData(ExcelWorksheet worksheet)
        {
            ...
            {
                productAttributeManager.SetSelectList("AttributeControlType", AttributeControlType.TextBox.ToSelectListInServices(useLocalization: false));
                productAttributeManager.SetSelectList("AttributeValueType", AttributeValueType.Simple.ToSelectListInServices(useLocalization: false));

                specificationAttributeManager.SetSelectList("AttributeType", SpecificationAttributeType.Option.ToSelectListInServices(useLocalization: false));
                ...
                manager.SetSelectList("ProductType", ProductType.SimpleProduct.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("GiftCardType", GiftCardType.Virtual.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("DownloadActivationType",
                    DownloadActivationType.Manually.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("ManageInventoryMethod",
                    ManageInventoryMethod.DontManageStock.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("LowStockActivity",
                    LowStockActivity.Nothing.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("BackorderMode", BackorderMode.NoBackorders.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("RecurringCyclePeriod",
                    RecurringProductCyclePeriod.Days.ToSelectListInServices(useLocalization: false));
                manager.SetSelectList("RentalPricePeriod", RentalPricePeriod.Days.ToSelectListInServices(useLocalization: false));
        ...
        public virtual void ImportProductsFromXlsx(Stream stream)
        {
            ...
            {
                throw new NopException("Invalid template");
            }
            if (metadata.Properties[1].StringValue=="Name")
            {
                throw new NopException("Invalid template");
            }
            ...
            var typeOfExportedAttribute = ExportedAttributeType.NotSpecified;
            if (metadata.EndRow == 2)
            {
                throw new NopException("No Data found");
            }
            for (var iRow = 2; iRow < metadata.EndRow; iRow++)
            {
                ...
                if (string.IsNullOrEmpty(metadata.Manager.GetProperty("Name").StringValue))
                {
                    throw new ArgumentNullException("Name is Empty");
                }
                if (string.IsNullOrEmpty(metadata.Manager.GetProperty("Price").StringValue))
                {
                    throw new ArgumentNullException("Price is Empty");
                }
                if (string.IsNullOrEmpty(metadata.Manager.GetProperty("FullDescription").StringValue))
                {
                    throw new ArgumentNullException("Description is Empty");
                }
                ...
                foreach (var property in metadata.Manager.GetProperties)
                {
                    switch (property.PropertyName)
                    {
                        ...
                        case "ShortDescription":
                            ...
                            break;
                        case "Categories":
                            categoryname = property.StringValue;
                            break;
                        ...
                        case "Vendor":
                            ...
                            if (_customerService.IsAdmin(_workContext.CurrentCustomer))
                                sellername = property.StringValue;
                            break;
                ...
                if (isNew && _customerService.IsVendor(_workContext.CurrentCustomer))
                    product.VendorId = _workContext.CurrentVendor.Id;

                if (isNew && _customerService.IsAdmin(_workContext.CurrentCustomer))
                {
                    //var vendors=_vendorService.GetAllVendors(VendorRegisterationType.Merchant,name: sellername);
                    //var vendor = vendors != null ? vendors[0] : null;
                    //if (vendor != null)
                    //{
                    //    product.VendorId = vendor.Id;
                    //}
                    product.VendorId = sellername!=null?Convert.ToInt32(sellername): 0;
                ...
                if (isNew)
                {
                    var categories = _categoryService.GetAllCategories(categoryName: categoryname);

                    var category = categories!=null?categories[0]:null;
        ...
        public virtual void ImportProductsOldSystemFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemCategoryMetaData>("id"),
                new PropertyByName<OldSystemCategoryMetaData>("title"),
                new PropertyByName<OldSystemCategoryMetaData>("description"),
                new PropertyByName<OldSystemCategoryMetaData>("price"),
                new PropertyByName<OldSystemCategoryMetaData>("discountPrice"),
                new PropertyByName<OldSystemCategoryMetaData>("discountPercentage"),
                new PropertyByName<OldSystemCategoryMetaData>("discountDate"),
                new PropertyByName<OldSystemCategoryMetaData>("expiredDate"),
                new PropertyByName<OldSystemCategoryMetaData>("offerID"),
                new PropertyByName<OldSystemCategoryMetaData>("storeID"),
                new PropertyByName<OldSystemCategoryMetaData>("categoryID"),
                new PropertyByName<OldSystemCategoryMetaData>("status"),
                new PropertyByName<OldSystemCategoryMetaData>("createdDate"),
            };
            
            var manager = new PropertyManager<OldSystemCategoryMetaData>(properties, _catalogSettings);
            ...
            using (var xlPackage = new ExcelPackage(stream))    
            {
                ...
                while (true)
                {
                    ...
                    var product = _productService.GetProductByOldSystemId(manager.GetProperty("id").IntValue);
                    var isNew = product == null;
                    product = product ?? new Product();

                    product.OldSystemProductId = manager.GetProperty("id").IntValue;
                    product.Name = manager.GetProperty("title").StringValue;
                    var description = manager.GetProperty("description").StringValue;
                    if (!string.IsNullOrWhiteSpace(description))
                        product.FullDescription = description;

                    var price = manager.GetProperty("price").DecimalValue;
                    var discountPrice = manager.GetProperty("discountPrice").DecimalValue;
                    ...
                    try
                    {
                        ...
                        int status = manager.GetProperty("status").IntValue;
                        switch (status)
                        {
                            //publishede
                            case 4:
                        ...
                        int categoryId = manager.GetProperty("categoryID").IntValue;
                        Category category = _categoryService.GetCategoryByOldSystemId(categoryId);
                        if (category != null)
                        {
                            var selectedCategoryIds = new List<int>() { category.Id };
                            //get its parent if it has
                            if (category.ParentCategoryId > 0)
                            {
                                var parentCategory = _categoryService.GetCategoryById(category.ParentCategoryId);
                                if (parentCategory != null)
                                {
                                    selectedCategoryIds.Add(parentCategory.Id);
                                }
                            }
                            ...
                            foreach (var catId in selectedCategoryIds)
                                if (_categoryService.FindProductCategory(existingProductCategories,product.Id, catId) == null)
                                {
                                    //find next display order
                                    var displayOrder = 1;
                                    var existingCategoryMapping = _categoryService.GetProductCategoriesByCategoryId(catId, showHidden: true);
                        ...
                        //vendor
                        int storeId = manager.GetProperty("storeID").IntValue;
                        if (storeId > 0)
                        {
                            Vendor vendor = _vendorService.GetVendorByOldSystemId(storeId);
                            if (vendor != null)
                            {
                                product.VendorId = vendor.Id;
                                _productService.UpdateProduct(product);
                    ...
                    {
                        _logger.Error(string.Format("Product Import Error: OldSystemProductId#{0}, Name#{1}", product.OldSystemProductId, product.Name), ex);
                    }
                    iRow++;
                }
            }
        ...
        public virtual void ImportOldSystemPicturesFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemPictureMetaData>("id"),
                new PropertyByName<OldSystemPictureMetaData>("path"),
                new PropertyByName<OldSystemPictureMetaData>("last_update"),
                new PropertyByName<OldSystemPictureMetaData>("item_id"),
                new PropertyByName<OldSystemPictureMetaData>("picname")
            };

            var manager = new PropertyManager<OldSystemPictureMetaData>(properties,_catalogSettings);
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                while (true)
                {
                    ...
                    int oldSysProductId = manager.GetProperty("item_id").IntValue;
                    if (oldSysProductId > 0)
                    {
                        var product = _productService.GetProductByOldSystemId(oldSysProductId);
                        if (product != null)
                        {
                            try
                            {
                                int oldSysPicId = manager.GetProperty("id").IntValue;
                                var picture = _pictureService.GetPictureByOldSystemId(oldSysPicId);
                                var isNew = picture == null;
                                picture = picture ?? new Picture();

                                picture.OldSystemPictureId = oldSysPicId;
                                //picture
                                var imagePath = manager.GetProperty("path").StringValue;
                                if (!string.IsNullOrEmpty(imagePath))
                                {
                                    var imageName = imagePath + ".jpg";
                                    var baseUrl = "https://uaegwu.com/pic/" + imageName;
                                    HttpWebRequest imageRequest = (HttpWebRequest)WebRequest.Create(baseUrl);
                                    WebResponse imageResponse = imageRequest.GetResponse();
                                    byte[] fileBinary = null;
                                    if (imageResponse.ContentType == MimeTypes.ImageJpeg)
                                    {
                                    ...
                                    if (isNew)
                                    {
                                        if (fileBinary != null)
                                        {
                                            var newPicture = _pictureService.InsertPicture(fileBinary, MimeTypes.ImageJpeg, null,
                                                picture.OldSystemPictureId);
                                            _productService.InsertProductPicture(new ProductPicture
                                            {
                                                PictureId = newPicture.Id,
                                                ProductId = product.Id,
                            ...
                            {
                                _logger.Error(string.Format("Picture Import Error: ProductId#{0}", product.Id), ex);
                            }
                        }
                    }
                    iRow++;
        ...
        public virtual void ImportOldSystemCategoriesFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemCategoryMetaData>("id"),
                new PropertyByName<OldSystemCategoryMetaData>("title"),
                new PropertyByName<OldSystemCategoryMetaData>("last_update"),
                new PropertyByName<OldSystemCategoryMetaData>("parent")
            };
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                while (true)
                {
                    ...
                    var category = _categoryService.GetCategoryByOldSystemId(manager.GetProperty("id").IntValue);
                    var isNew = category == null;
                    category = category ?? new Category();

                    category.OldSystemCategoryId = manager.GetProperty("id").IntValue;
                    category.Name = manager.GetProperty("title").StringValue;
                    category.CategoryTemplateId = 1;
                    int oldSysParentCategoryId = manager.GetProperty("parent").IntValue;
                    if (oldSysParentCategoryId > 0)
                    {
                        var parentCategory = _categoryService.GetCategoryByOldSystemId(oldSysParentCategoryId);
                        if (parentCategory != null)
                            category.ParentCategoryId = parentCategory.Id;
                    }
                    ...
                    if (isNew)
                    {
                        ...
                        var seName = _urlRecordService.ValidateSeName(category, "", category.Name, true);// category.ValidateSeName("", category.Name, true);
                    ...
                    else
                    {
                        _categoryService.UpdateCategory(category);
                        //search engine name
                        var seName = _urlRecordService.ValidateSeName(category,_urlRecordService.GetSeName(category, 0), category.Name, true);
                        _urlRecordService.SaveSlug(category, seName, 0);
                    }
                    iRow++;
                }
        ...
        public virtual void ImportCustomerFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemCustomerMetaData>("id"),
                new PropertyByName<OldSystemCustomerMetaData>("username"),
                new PropertyByName<OldSystemCustomerMetaData>("name"),
                new PropertyByName<OldSystemCustomerMetaData>("password"),
                new PropertyByName<OldSystemCustomerMetaData>("type"),
                new PropertyByName<OldSystemCustomerMetaData>("eduLevel"),
                new PropertyByName<OldSystemCustomerMetaData>("nationality"),
                new PropertyByName<OldSystemCustomerMetaData>("email"),
                new PropertyByName<OldSystemCustomerMetaData>("phone"),
                new PropertyByName<OldSystemCustomerMetaData>("mobile"),
                new PropertyByName<OldSystemCustomerMetaData>("mobile2"),
                new PropertyByName<OldSystemCustomerMetaData>("fax"),
                new PropertyByName<OldSystemCustomerMetaData>("status"),
                new PropertyByName<OldSystemCustomerMetaData>("create_date"),
                new PropertyByName<OldSystemCustomerMetaData>("last_update"),
                new PropertyByName<OldSystemCustomerMetaData>("addedBy"),
                new PropertyByName<OldSystemCustomerMetaData>("token")
            };

            var manager = new PropertyManager<OldSystemCustomerMetaData>(properties,_catalogSettings);
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                while (true)
                {
                    ...
                    var username = manager.GetProperty("username").StringValue;
                    Customer customer = _customerService.GetCustomerByUsername(username);

                    var isNew = customer == null;
                    customer = customer ?? new Customer();

                    int id = manager.GetProperty("id").IntValue;
                    customer.CustomerOldSystemId = id;
                    int type = manager.GetProperty("type").IntValue;
                    customer.CustomerOldSystemType = type;

                    customer.Username = username;

                    string createDate = manager.GetProperty("create_date").StringValue;
                    if (!string.IsNullOrEmpty(createDate))
                    {
                        DateTime parsedCreateDate;
                        bool isParsed = DateTime.TryParse(createDate, out parsedCreateDate);
                        if (isParsed)
                            customer.CreatedOnUtc = parsedCreateDate;
                        else
                    ...
                    string email = manager.GetProperty("email").StringValue;
                    ...
                    //password
                    string password = manager.GetProperty("password").StringValue;
                    if (!string.IsNullOrWhiteSpace(password) && !string.IsNullOrWhiteSpace(email))
                    {
                        if (CommonHelper.IsValidEmail(email))
                        {
                            var changePassRequest = new ChangePasswordRequest(email, false, _customerSettings.DefaultPasswordFormat, password);
                            var changePassResult = _customerRegistrationService.ChangePassword(changePassRequest);
                            if (!changePassResult.Success)
                            {
                                foreach (var changePassError in changePassResult.Errors)
                                    throw new Exception(changePassError);
                            }
                        }
                    }

                    //attributes
                    string name = manager.GetProperty("name").StringValue;
                    _genericAttributeService.SaveAttribute<string>(customer, NopCustomerDefaults.FirstNameAttribute, name);
                    
                    int nationality = manager.GetProperty("nationality").IntValue;
                    switch (nationality)
                    {
                        //Jordan
                        case 576:
                    ...
                    _genericAttributeService.SaveAttribute(customer, NopCustomerDefaults.NationalityCountryId, nationality);
                    _genericAttributeService.SaveAttribute<string>(customer, NopCustomerDefaults.CountryIdAttribute, nationality.ToString());

                    string phone = manager.GetProperty("phone").StringValue;
                    if (!string.IsNullOrWhiteSpace(phone))
                        _genericAttributeService.SaveAttribute<string>(customer, NopCustomerDefaults.PhoneAttribute, phone);

                    //do not duplicate address address
                    if (isNew || customer.Addresses.Count == 0)
                    {
                        string mobile = manager.GetProperty("mobile").StringValue;
                        string mobile2 = manager.GetProperty("mobile2").StringValue;
                        if (string.IsNullOrWhiteSpace(mobile))
                            mobile = mobile2;

                        //insert default address (if possible)
                        var defaultAddress = new Core.Domain.Common.Address
                        {
                            FirstName = _genericAttributeService.GetAttribute<Customer,string>(customer.Id, NopCustomerDefaults.FirstNameAttribute),
                            Email = customer.Email,
                            CountryId = _genericAttributeService.GetAttribute<Customer,int>(customer.Id,NopCustomerDefaults.CountryIdAttribute) > 0 ? (int?)_genericAttributeService.GetAttribute<Customer, int>(customer.Id, NopCustomerDefaults.CountryIdAttribute) : null,
                            PhoneNumber = mobile,
                    ...
                    //customer role
                    if (customer.CustomerRoles.Count == 0 && customer.CustomerRoles.FirstOrDefault(cr => cr.SystemName == NopCustomerDefaults.RegisteredRoleName) == null)
                    {
                        CustomerRole customerRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.RegisteredRoleName);
                        //Valid Email is required for customer to be in 'Registered' role
                        if (!CommonHelper.IsValidEmail(email))
                            email = "default" + customer.CustomerOldSystemId + "@gwu.ae";
        ...
        public virtual void ImportAddressFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemCustomerMetaData>("id"),
                new PropertyByName<OldSystemCustomerMetaData>("address"),
                new PropertyByName<OldSystemCustomerMetaData>("city_id"),
                new PropertyByName<OldSystemCustomerMetaData>("postal_code"),
                new PropertyByName<OldSystemCustomerMetaData>("phone"),
                new PropertyByName<OldSystemCustomerMetaData>("userID"),
                new PropertyByName<OldSystemCustomerMetaData>("last_update")
            };
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                while (true)
                {
                    ...
                    int userId = manager.GetProperty("userID").IntValue;
                    if (userId > 0)
                    {
                        Customer customer = _customerService.GetCustomerByOldSystemCustomerId(userId);
                        if (customer != null)
                        {
                            int cityId = manager.GetProperty("city_id").IntValue;
                            ...
                            if (stateProvinceId > 0)
                                _genericAttributeService.SaveAttribute<string>(customer, NopCustomerDefaults.StateProvinceIdAttribute, stateProvinceId.ToString());

                            Address currentAddress = customer.Addresses.FirstOrDefault();
                            if (currentAddress != null)
                            {
                                var oldSysAddress = manager.GetProperty("address").StringValue;
                                if (!string.IsNullOrEmpty(oldSysAddress) && !string.Equals(oldSysAddress, "NULL"))
                                {
                                    currentAddress.Address1 = oldSysAddress;
                                    _addressService.UpdateAddress(currentAddress);
                                }
                            }
                        }
                    }
        ...
        public virtual void ImportVendorFromXlsx(Stream stream)
        {
            var properties = new[]
            {
                new PropertyByName<OldSystemVendorMetaData>("storeID"),
                new PropertyByName<OldSystemVendorMetaData>("title"),
                new PropertyByName<OldSystemVendorMetaData>("description"),
                new PropertyByName<OldSystemVendorMetaData>("manager_staff_id"),
                new PropertyByName<OldSystemVendorMetaData>("storeStatus"),
                new PropertyByName<OldSystemVendorMetaData>("profileImg"),
                new PropertyByName<OldSystemVendorMetaData>("last_update"),
            };

            var manager = new PropertyManager<OldSystemVendorMetaData>(properties,_catalogSettings);
            using (var xlPackage = new ExcelPackage(stream))
            {
                // get the first worksheet in the workbook
                var worksheet = xlPackage.Workbook.Worksheets.FirstOrDefault();
                ...
                while (true)
                {
                    ...
                    var storeId = manager.GetProperty("storeID").IntValue;
                    Vendor vendor = _vendorService.GetVendorByOldSystemId(storeId);
                    var isNew = vendor == null;
                    vendor = vendor ?? new Vendor();

                    vendor.VendorOldSystemId = storeId;
                    string title = manager.GetProperty("title").StringValue;
                    //check for english and arabic
                    //if (Regex.IsMatch(title, "^[a-zA-Z0-9]*$"))
                    //{                    //}
                    vendor.Name = title;
                    string description = manager.GetProperty("description").StringValue;
                    if (!string.IsNullOrEmpty(description) && !string.Equals("NULL", description, StringComparison.OrdinalIgnoreCase))
                        vendor.Description = description;

                    int storeStatus = manager.GetProperty("storeStatus").IntValue;
                    switch (storeStatus)
                    {
                        case 4:
                            vendor.Active = true;
                    ...
                    //picture
                    var imageName = storeId + ".jpg";
                    var baseUrl = "https://uaegwu.com/store/" + imageName;
                    ...
                    //customer
                    int managerStaffId = manager.GetProperty("manager_staff_id").IntValue;
                    Customer customer = _customerService.GetCustomerByOldSystemCustomerId(managerStaffId);
                    if (customer != null)
                    {
                        customer.VendorId = vendor.Id;
                        vendor.Email = customer.Email;
                        if (customer.CustomerOldSystemType.HasValue)
                        {
                            switch (customer.CustomerOldSystemType.Value)
                            {
                                case 2:
                                    //Merchant
                                    vendor.RegisterationType = VendorRegisterationType.Merchant;
                                    if (customer.CustomerRoles.FirstOrDefault(cr => cr.SystemName == NopCustomerDefaults.VendorsRoleName) == null)
                                    {
                                        CustomerRole customerRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.VendorsRoleName);
                                        customer.CustomerRoles.Add(customerRole);
                                    }
                                    break;
                                case 3:
                                    ...
                                    vendor.FoundationTypeStatus = FoundationTypeStatus.CannotAproveOrRejectRequest;
                                    if (customer.CustomerRoles.FirstOrDefault(cr => cr.SystemName == NopCustomerDefaults.Foundations) == null)
                                    {
                                        CustomerRole customerRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.Foundations);
                                        customer.CustomerRoles.Add(customerRole);
                                    }
                                    break;
                                default:
        ...
        protected class OldSystemCustomerMetaData
        {
        ...
        protected class OldSystemVendorMetaData
        {
        ...
        protected class OldSystemCategoryMetaData
        {
            public int id { get; set; }
            public string title { get; set; }
        ...
        protected class OldSystemPictureMetaData
        {
            public int id { get; set; }
