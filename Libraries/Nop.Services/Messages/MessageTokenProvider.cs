...
namespace Nop.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        #region Methods
        public virtual void AddContactUsTokens(IList<Token> tokens, string subject, string email, string body)
        {
            tokens.Add(new Token("ContactUs.Subject", subject, true));
            tokens.Add(new Token("ContactUs.Email", email, true));
            tokens.Add(new Token("ContactUs.Body", body, true));
        }

        public virtual void AddCampaignTokens(IList<Token> tokens, Campaign campaign, int langaugeId)
        {
            if (campaign == null)
                throw new ArgumentNullException("campaign");

            tokens.Add(new Token("Campaign.Id", campaign.Id.ToString(), true));
            tokens.Add(new Token("Campaign.Name", _localizationService.GetLocalized(campaign, x => x.Name, langaugeId), true));
            tokens.Add(new Token("Campaign.Subject", _localizationService.GetLocalized(campaign, x => x.Subject,langaugeId), true));

            //event notification
            _eventPublisher.EntityTokensAdded(campaign, tokens);
        }

        public virtual void AddFoundationTokens(IList<Token> tokens, Foundation foundation)
        {
            if (foundation == null)
                throw new ArgumentNullException("foundation");

            tokens.Add(new Token("Foundation.Id", foundation.Id.ToString(), true));
            tokens.Add(new Token("Foundation.Name", foundation.Name, true));
            tokens.Add(new Token("Foundation.Email", foundation.Email, true));

            //event notification
            _eventPublisher.EntityTokensAdded(foundation, tokens);
        }
        public virtual void AddVendorLicenseTokens(IList<Token> tokens, Vendor vendor,string ExpiryDate, int langaugeId)
        {
            if (vendor == null)
                throw new ArgumentNullException("vendor");
            tokens.Add(new Token("vendor.Name", vendor.Name, true));
            tokens.Add(new Token("vendor.ExpiryDate", ExpiryDate, true));

            //event notification
            _eventPublisher.EntityTokensAdded(vendor, tokens);
        }

        public virtual void AddBlackPointTokens(IList<Token> tokens, BlackPoint blackPoint)
        {
            if (blackPoint == null)
            ...
            //tokens.Add(new Token("Foundation.Id", foundation.Id.ToString(), true));
            //tokens.Add(new Token("Foundation.Name", foundation.Name, true));
            string vendorOrCustomer = string.Empty;
            string vendorOrCustomerType = string.Empty;
            if (blackPoint.AddedOnTypeId == (int)BlackPointTypeEnum.Customer)
            {
                int customerId = blackPoint.VendorOrCustomerId;
                var customer = _customerService.GetCustomerById(customerId);
                if (customer != null)
                    vendorOrCustomer = _customerService.GetCustomerFullName(customer);

                vendorOrCustomerType = _localizationService.GetResource("enums.Nop.Core.Domain.BlackPoints.BlackPointTypeEnum.Customer");
            }
            else if (blackPoint.AddedOnTypeId == (int)BlackPointTypeEnum.Vendor)
            {
                int vendorId = blackPoint.VendorOrCustomerId;
                var vendor = _vendorService.GetVendorById(vendorId);
                if (vendor != null)
                    vendorOrCustomer = _localizationService.GetLocalized(vendor,v => v.Name);

                vendorOrCustomerType = _localizationService.GetResource("enums.Nop.Core.Domain.BlackPoints.BlackPointTypeEnum.Vendor");
            }

            var editUrl = string.Format("{0}Admin/BlackPoint/Edit/{1}", GetStoreUrl(0), blackPoint.Id);
            tokens.Add(new Token("blackpoint.vendorOrCustomer", vendorOrCustomer, true));
            tokens.Add(new Token("blackpoint.vendorOrCustomerType", vendorOrCustomerType, true));
            tokens.Add(new Token("blackpoint.url", editUrl, true));
            //event notification
            _eventPublisher.EntityTokensAdded(blackPoint, tokens);
        }
        ...
        public virtual void AddCustomerTokens(IList<Token> tokens, int customerId, string reason = null)
        {
            if (customerId <= 0)
                throw new ArgumentOutOfRangeException(nameof(customerId));
            ...
            AddCustomerTokens(tokens, customer, reason: reason);
        }
        ...
        /// <param name="tokens">List of already added tokens</param>
        /// <param name="customer">Customer</param>
        /// <param name="reason">Customer</param>
        public virtual void AddCustomerTokens(IList<Token> tokens, Customer customer, string reason=null)
        {
            tokens.Add(new Token("Customer.Email", customer.Email));
            tokens.Add(new Token("Customer.Username", customer.Username));
            tokens.Add(new Token("Customer.FullName", _customerService.GetCustomerFullName(customer)));
            ...
            tokens.Add(new Token("Customer.VatNumber", _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.VatNumberAttribute)));
            tokens.Add(new Token("Customer.VatNumberStatus", ((VatNumberStatus)_genericAttributeService.GetAttribute<int>(customer, NopCustomerDefaults.VatNumberStatusIdAttribute)).ToString()));
            tokens.Add(new Token("Customer.Reason",!string.IsNullOrEmpty(reason)? reason:_genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.customerDeleteAccountReasonAttribute), true));

            string CustomerType = _customerService.IsVendor(customer) ? _localizationService.GetLocaleStringResourceByName("usertype.seller").ResourceValue : _localizationService.GetLocaleStringResourceByName("usertype.buyer").ResourceValue;
            tokens.Add(new Token("Customer.Type", CustomerType));
            var customAttributesXml = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.CustomCustomerAttributes);
            tokens.Add(new Token("Customer.CustomAttributes", _customerAttributeFormatter.FormatAttributes(customAttributesXml), true));
            //tokens.Add(new Token("Customer.DeleteAccountType", _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.DeleteAccountType)));
            //note: we do not use SEO friendly URLS for these links because we can get errors caused by having .(dot) in the URL (from the email address)
            var passwordRecoveryUrl = RouteUrl(routeName: "PasswordRecoveryConfirm", routeValues: new { token = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.PasswordRecoveryTokenAttribute), guid = customer.CustomerGuid });
            var accountActivationUrl = RouteUrl(routeName: "AccountActivation", routeValues: new { token = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.AccountActivationTokenAttribute), guid = customer.CustomerGuid });
            var emailRevalidationUrl = RouteUrl(routeName: "EmailRevalidation", routeValues: new { token = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.EmailRevalidationTokenAttribute), guid = customer.CustomerGuid });
        ...
        public virtual void AddVendorTokens(IList<Token> tokens, Vendor vendor)
        {
            ...
            var Reason= _genericAttributeService.GetAttribute<string>(vendor, NopCustomerDefaults.customerDeleteAccountReasonAttribute);
            if (string.IsNullOrEmpty(Reason) && customer!=null)
            {
                Reason= _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.customerDeleteAccountReasonAttribute);
            }
            tokens.Add(new Token("Vendor.Name", _customerService.GetCustomerFullName(customer)));
            tokens.Add(new Token("Vendor.Email", vendor.Email));
            tokens.Add(new Token("Vendor.Reason", Reason, true));
        ...
        public virtual void AddVendorOrCustomerTokens(IList<Token> tokens, Vendor vendor, Customer customer)
        {
            if (vendor != null)
            {
                tokens.Add(new Token("CustomerName", vendor.Name));
                //event notification
                _eventPublisher.EntityTokensAdded(vendor, tokens);
            }
            else if (customer != null)
            {
                tokens.Add(new Token("CustomerName", _customerService.GetCustomerFullName(customer)));
                //event notification
                _eventPublisher.EntityTokensAdded(customer, tokens);
            }
        }
