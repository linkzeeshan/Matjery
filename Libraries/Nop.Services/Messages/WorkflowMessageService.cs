...
namespace Nop.Services.Messages
{
    ...
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        private readonly IQueuedPushNotificationService _queuedPushNotification;
        ...
        public WorkflowMessageService(CommonSettings commonSettings,
           // IGenericAttributeService genericAttributeService,
            ...
            IQueuedPushNotificationService queuedPushNotification,
            ...
            )
        {
            ...
            _queuedPushNotification = queuedPushNotification;
        ...
        {
            var messageTemplate = _messageTemplateService.GetMessageTemplatesByName(messageTemplateName, storeId);

            //no template found
            if (!messageTemplate?.Any() ?? true)
                return new MessageTemplate();

            //filter active templates
            messageTemplate = messageTemplate.Where(messageTemplate => messageTemplate.IsActive).ToList();
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    attachmentFilePath, attachmentFileName,customer:customer);
            }).ToList();
        }
        ...
        /// 
        //
        public virtual IList<int> SendOrderProcessesCustomerNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderProcessedCustomerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
            ...
            {
                _messageTokenProvider.AddOrderNoteTokens(commonTokens, order.OrderNotes.FirstOrDefault());
            }
        ...
        public virtual int SendContactUsNotification(int languageId, string subject, string email, string body)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("Store.ContactUs", store.Id);
            ...
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddContactUsTokens(tokens, subject, email, body);
        ...
        /// Sends 'Vendor AccountApproved' message to a store owner
        /// </summary>
        /// <param name="vendor">Vendor</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendVendorAccountApprovedNotification(Vendor vendor, int languageId)
        {
            if (vendor == null)
            ...
            var messageTemplate = GetActiveMessageTemplate("Vendor.AccountApproved", store.Id);
            if (messageTemplate == null)
                return 0;
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendVendorproductpublishedeNotification(Vendor vendor, Product product, int languageId)
        {
            if (vendor == null)
                throw new ArgumentNullException("vendor");
            if (product == null)
            ...
            var messageTemplate = GetActiveMessageTemplate("Vendor.ProductPublished", store.Id);
            if (messageTemplate == null)
                return 0;
        ...
        public virtual IList<int> SendProductReviewNotificationMessage(ProductReview productReview, int languageId)
        {
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                var vendor = _vendorService.GetVendorByProductId(productReview.ProductId);
                var customer = _customerService.GetCustomerByVendorId(vendor.Id);
                var toEmail = customer.Email;
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName, customer: customer);
            }).ToList();
        }
        ...
        public virtual IList<int> SendContactVendorMessage(Vendor vendor, int languageId, string senderEmail,
             string senderName, string inquiry, string subject, string body)
        {
            if (vendor == null)
                throw new ArgumentNullException(nameof(vendor));
            ...
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);
            body = $"Dear {WebUtility.HtmlEncode(vendor.Name)}, <br /><br /> You have received an inquiry from {WebUtility.HtmlEncode(senderName)}. <br /><br />{WebUtility.HtmlEncode(inquiry)}<br /><br /><br />Regards, <br/>Matjery";
        ...
        public virtual int SendNotification(MessageTemplate messageTemplate,
            ...
            string fromEmail = null, string fromName = null, string subject = null,Customer customer=null)
        {
            if (messageTemplate == null)
            ...
            _queuedEmailService.InsertQueuedEmail(email);
             ...
             var template = _pushNotificationTemplateService.GetActivePushNotificationTemplate(messageTemplate.Name);
            if (template != null)
            {

                //gene
                customer = customer==null?_customerService.GetCustomerByEmail(toEmailAddress): customer;
                var _genericAttributeService = EngineContext.Current.Resolve<IGenericAttributeService>();
                int CustomerLanguae = 0;
                if (customer != null)
                {
                    CustomerLanguae = _genericAttributeService.GetAttribute<int>(customer, NopCustomerDefaults.LanguageIdAttribute, _storeContext.CurrentStore.Id);
                }
            
                string QPsubject = _localizationService.GetLocalized(template, mt => mt.Title, CustomerLanguae>0?CustomerLanguae:languageId);
                var QPbody = _localizationService.GetLocalized(template, mt => mt.Message, CustomerLanguae > 0 ? CustomerLanguae : languageId);

                //Replace subject and body tokens 
                var QPsubjectReplaced = _tokenizer.Replace(QPsubject, tokens, false);
                var QPbodyReplaced = _tokenizer.Replace(QPbody, tokens, true);

                var pushNotification = new QueuedPushNotification
                {
                    CreatedOnUtc = DateTime.UtcNow,
                    Message = Regex.Replace(QPbodyReplaced, "<.*?>", String.Empty), //template.Message, 
                    Title = Regex.Replace(QPsubjectReplaced, "<.*?>", String.Empty), //template.Title,
                ...
                _queuedPushNotification.QueuePushNotificationData(pushNotification.Title, pushNotification.Message, email: toEmailAddress,  customer :customer);
            }
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendApplyFoundationSubmitNotificationMessage(Foundation foundation, int languageId)
        {
            if (foundation == null)
            ...
            var messageTemplate = GetActiveMessageTemplate("FoundationApply.AdminNotification", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddFoundationTokens(tokens, foundation);
        ...
        public int SendBlackPointNotificationToAdmin(BlackPoint blackPoint, int languageId)
        {
            if (blackPoint == null)
            ...
            var messageTemplate = GetActiveMessageTemplate("BlackPoint.AdminNotification", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddBlackPointTokens(tokens, blackPoint);
        ...
        public int SendBlackPointBlockedNotificationToVendor(Vendor vendor, int languageId, bool addNotification = false,Order order=null)
        {
            if (vendor == null)
                throw new ArgumentNullException("vendor");
            ...
            if (addNotification)
                messageTemplate = GetActiveMessageTemplate("BlackPoint.CustomerOrVendor", store.Id);
            else
                messageTemplate = GetActiveMessageTemplate("BlackPoint.VendorOrCustomerNotification", store.Id);
            ...
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddVendorTokens(tokens, vendor);
            _messageTokenProvider.AddVendorOrCustomerTokens(tokens, vendor, null);
            if (order != null)
            {
            ...
            var toEmail = vendor.Email;
            var toName = _localizationService.GetLocalized(vendor, x => x.Name);
            var customer = _customerService.GetCustomerByVendorId(vendor.Id);
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName, customer: customer);
        }

        public int SendBlackPointBlockedNotificationToCustomer(Customer customer, int languageId, bool addNotification = false,Order order=null)
        {
            if (customer == null)
            ...
            _messageTokenProvider.AddVendorOrCustomerTokens(tokens, null, customer);
            if (order != null)
            {
        ...
        public MessageTemplate GetNotificationEmailTemplate(int storeId, int languageId)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("Notification.Email", store.Id);
        ...
        public int SendBlackPointApprovedNotificationToCustomerOrVendor(string email, string name, int languageId)
        {
            ...
            var messageTemplate = GetActiveMessageTemplate("BlackPoint.VendorOrCustomerNotificationApproved", store.Id);
        ...
        public IList<int> SendCustAccountDeletedNotification(Customer customer, int languageId)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.CustomerAccountDeletedMessage, store.Id);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    fromEmail: fromEmail,
                    fromName: fromName,
                    replyToEmailAddress: customer.Email,
                    replyToName: customer.GetFullName());
            }).ToList();
        }

        public IList<int> SendSellerAccountDeletedNotification(Vendor vendor, int languageId)
        {
            if (vendor == null)
                throw new ArgumentNullException(nameof(vendor));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.VendorAccountDeletedMessage, store.Id);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    fromEmail: fromEmail,
                    fromName: fromName,
                    replyToEmailAddress: vendor.Email,
                    replyToName: vendor.Name);
            }).ToList();
        }


        public IList<int> SendCustomerAccountDeleteNotification(Customer customer, int languageId, string subject, string email, string body, string reason = null)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.CustomerDeleteAccountMessage, store.Id);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                var toEmail = customer.Email;
                var toName = customer.GetFullName();
                var sendEmailToAdmin = SendDeleteAccountRequestToAdminNotification(customer.Email, customer.GetFullName(), customer: customer, languageId: languageId, reason: reason);
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    fromEmail: fromEmail,
                    fromName: fromName,
                    replyToEmailAddress: customer.Email,
        ...
        public IList<int> SendVendorAccountDeleteNotification(Vendor vendor, int languageId, string subject, string email, string body, string reason = null)
        {
            if (vendor == null)
                throw new ArgumentNullException(nameof(vendor));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.VendorDeleteAccountMessage, store.Id);
           ...
           // _messageTokenProvider.AddCustomerTokens(commonTokens, customer);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                var toEmail = vendor.Email;
                var toName = vendor.Name;
                var sendEmailToAdmin = SendDeleteAccountRequestToAdminNotification(vendor.Email, vendor.Name, vendor: vendor, languageId, reason: reason);
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    fromEmail: fromEmail,
                    fromName: fromName,
                    replyToEmailAddress: vendor.Email,
        ...
        public List<int> SendDeleteAccountRequestToAdminNotification(string email, string name, Customer customer = null, int languageId = 1, string reason = null)
        {
            if (name == null)
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.CustomerDeleteAccountRequestToAdminMessage, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();

            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddCustomerTokens(commonTokens, customer.Id, reason:reason);
           // _messageTokenProvider.AddCustomerTokens(commonTokens, customer.Id);
           // _messageTokenProvider.AddCustomerTokens(tokens, customer.Id);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                var toEmail = _commonSettings.AdminEmail;
                var toName = emailAccount.DisplayName;
        ...
        public List<int> SendDeleteAccountRequestToAdminNotification(string email, string name, Vendor vendor = null, int languageId = 1, string reason = null)
        {
            if (name == null)
                throw new ArgumentNullException(nameof(name));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.VendorDeleteAccountRequestToAdminMessage, store.Id);
            if(vendor != null)
                messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.VendorDeleteAccountRequestToAdminMessage, store.Id);
        ...
        #region license
        public int SendTradeLicenseExpiryNotificationToVendor(Vendor vendor, int languageId, bool addNotification = false, string ExpiryDate = "", string Template = "")
        {
            if (vendor == null)
                throw new ArgumentNullException("vendor");
            ...
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddVendorLicenseTokens(commonTokens, vendor, ExpiryDate, _workContext.WorkingLanguage.Id);
