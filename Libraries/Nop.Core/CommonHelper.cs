...
namespace Nop.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        /// <param name="email">The email.</param>
        /// <returns></returns>
        public static string CustomerEmailFormat(string email, bool isdeleted= false)
        {
            var output = EnsureNotNull(email);
            output = output.Trim();

                if (output.Contains("deleted_"))
                {
                    return output.Replace("deleted_", "");
                }
                if (isdeleted)
                {
                    output = $"deleted_{email}";
                }

            return output;
        }
        public static string CustomerNameFormat(string Name, bool isdeleted= false)
        {
            var output = EnsureNotNull(Name);
                ...
                if (isdeleted)
                {
                  output = $"deleted_{Name}";
                }
            return output;
        }
        public static string CustomerPhoneNumberFormat(string phone, bool isdeleted = false)
        {
            var output = EnsureNotNull(phone);
