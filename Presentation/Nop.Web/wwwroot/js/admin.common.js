...
function AutoTranslate(inpvar, outputvar, isRich) {
    var _textToTranslate = isRich ? inpvar: $(inpvar).val();
    GlobalAutoTranslate(_textToTranslate).then(function (response) {
        if (isRich) {
            outputvar.setContent(response[0]["translations"][0]["text"]);
        }
        else {
            $(outputvar).val(response[0]["translations"][0]["text"]);
        }
    });
}
function GlobalAutoTranslate (inpcontrol){
    try {
        var _textToTranslate = inpcontrol;
        return $.ajax({
            url: rootAppPath+"Admin/Common/Translate",
            data: {
