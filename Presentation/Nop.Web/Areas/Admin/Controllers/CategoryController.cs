...
using Nop.Web.Areas.Admin.Infrastructure.AutoTranslation;
    ...
    public partial class CategoryController : BaseAdminController
    {
        ...
        public virtual IActionResult Create(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //Not save if already existing
                var isexisting = _categoryService.GetAllCategories(model.Name).ToList();
                if (isexisting.Count() != 0)
                {
                    var catlogExist = $"{_localizationService.GetResource("Admin.Catalog.Products.newcatalog")}{model.Name}{_localizationService.GetResource("Admin.Catalog.Products.categoryAlreadyExist")}";
                    var notificationCatmessag = $"{_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.categoryAlreadyExist")}";
                    _notificationService.SuccessNotification(notificationCatmessag, name: catlogExist);
                    return RedirectToAction("List");
                }
                ...
                var newcatlog = $"{_localizationService.GetResource("Admin.Catalog.Products.newcatalog")}{model.Name}{_localizationService.GetResource("Admin.Catalog.Products.newproductcreated")}";
                var notificationmessag = $"{_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Categories.Added")}";
                _notificationService.SuccessNotification(notificationmessag, name: newcatlog);
                //_toastNotification.Error(notificationmessag);
                if (!continueEditing)
                    return RedirectToAction("List");
        ...
        public virtual IActionResult Edit(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var newcatlog = $"{_localizationService.GetResource("Admin.Catalog.Products.newcatalog")} {_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.newproductcreated")}";
                var notificationmessag = $"{_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Categories.Updated")}";
                _notificationService.SuccessNotification(notificationmessag, name: newcatlog);
        ...
        public virtual IActionResult Delete(int id)
        {
            ...
            //var newcatlog = $"{_localizationService.GetResource("Admin.Catalog.Products.newcatalog")} {_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.newproductcreated")}";
            var notificationmessag = $"{_localizationService.GetLocalized(category, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Categories.Deleted")}";
            _notificationService.SuccessNotification(notificationmessag, name: category.Name);
        ...
        [HttpPost]
        public virtual IActionResult ImportOldSystemFromXlsx(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            try
            {
                if (importexcelfile != null && importexcelfile.Length > 0)
                {
                    _importManager.ImportOldSystemCategoriesFromXlsx(importexcelfile.OpenReadStream());
                }
                else
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
