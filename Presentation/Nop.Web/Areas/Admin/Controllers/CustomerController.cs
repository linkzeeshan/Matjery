...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        public CustomerController(CustomerSettings customerSettings,
            ...
            TaxSettings taxSettings, IImportManager importManager,
        ...
        [HttpPost]
        public IActionResult DeletedCustomersList(CustomerSearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();

            var model = _customerModelFactory.PrepareDeletedCustomerListModel(searchModel);
            return Json(model);
        }
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                customer.CreatedBy = _workContext.CurrentCustomer.Id;
                customer.UpdatedBy = _workContext.CurrentCustomer.Id;
                ...
                {
                    _genericAttributeService.SaveAttribute(customer, NopCustomerDefaults.CountryIdAttribute, model.CountryId);
                    _genericAttributeService.SaveAttribute(customer, NopCustomerDefaults.NationalityCountryId, model.CountryId);
                }
                ...
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Added")}";
        ...
        public virtual IActionResult Edit(int id)
        {
            ...
            if (customer == null) // old code was if (customer == null || customer.Deleted)
                return RedirectToAction("List");

            var model = new CustomerModel();
            if (customer.ExternalAuthenticationRecords.Count > 0)
            {
                foreach (var external in customer.ExternalAuthenticationRecords)
                {
                    if (external.ProviderSystemName == "ExternalAuth.UaePass")
                    {
            ...
            //prepare model
            model = _customerModelFactory.PrepareCustomerModel(model, customer,false,true);
        ...
        public virtual IActionResult Edit(CustomerModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Updated")}";
                    _notificationService.SuccessNotification(notificationmessag);
        ...
        public virtual IActionResult ChangePassword(CustomerModel model)
        {
            ...
            if (changePassResult.Success)
            {
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.PasswordChanged")}";
                _notificationService.SuccessNotification(notificationmessag);
            }
        ...
        public virtual IActionResult Delete(int id)
        {
            ...
            try
            {
                ...
                //verify Orders are in pending and processing sate so not delete
                var customerOrder = _customerModelFactory.PrepareCustomerOrderListModel(customer.Id, customer);
                if(customerOrder.Count() > 0)
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.AdminAccountShouldExists.OrderStatus"));
                    return RedirectToAction("Edit", new { id = customer.Id });
                }
                ...
                if (_customerService.IsVendor(customer))
                {
                ...
                var externalUser = _openAuthenticationService.UserExist(customer.Id, customer.Email);
                if (externalUser)
                ...
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Deleted")}";
                _notificationService.SuccessNotification(notificationmessag);
                customer.Email = Customeremail;
                _workflowMessageService.SendCustAccountDeletedNotification(customer: customer, languageId: _workContext.WorkingLanguage.Id);
                return RedirectToAction("List");
            }
            catch (Exception exc)
            {
        ...
        public virtual IActionResult SendWelcomeMessage(CustomerModel model)
        {
            ...
            var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.SendWelcomeMessage.Success")}";
            _notificationService.SuccessNotification(notificationmessag);
        ...
        public virtual IActionResult ReSendActivationMessage(CustomerModel model)
        {
            ...
            var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.ReSendActivationMessage.Success")}";
            _notificationService.SuccessNotification(notificationmessag);
        ...
        public virtual IActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                ...
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.SendPM.Sent")}";
                _notificationService.SuccessNotification(notificationmessag);
            }
            catch (Exception exc)
            {
        ...
        public virtual IActionResult AddressCreate(CustomerAddressModel model, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Addresses.Added")}";
                _notificationService.SuccessNotification(notificationmessag);
        ...
        public virtual IActionResult AddressEdit(CustomerAddressModel model, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Addresses.Updated")}";
                _notificationService.SuccessNotification(notificationmessag);
        ...
        #region Customer
        [HttpPost, ActionName("Edit")]
        public virtual IActionResult Retrievecustomer(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            //Check Email and user name Format Format
            ...
            _genericAttributeService.SaveAttribute(customer, NopCustomerDefaults.customerDeleteAccountAttribute, false);
            var notificationmessag = $"{customer.GetFullName()} {_localizationService.GetResource("Admin.Customers.Customers.Retrieved")}";
            _notificationService.SuccessNotification(notificationmessag);
            return RedirectToAction("Edit", new { id = customer.Id });
        ...
        [HttpPost]
        public IActionResult ImportExcelCustomer(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            try
            {
                ...
                {
                    _importManager.ImportCustomerFromXlsx(importexcelfile.OpenReadStream());
                }
                else
                {
        ...
        [HttpPost]
        public IActionResult ImportExcelAddress(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            try
            {
                if (importexcelfile != null && importexcelfile.Length > 0)
                {
                    _importManager.ImportAddressFromXlsx(importexcelfile.OpenReadStream());
                }
                else
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
