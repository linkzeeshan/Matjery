...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        private readonly ICustomerTranslatorService _customerTranslatorService;
        ...
        public ProductController(IAclService aclService,
            ...
            ICustomerTranslatorService CustomerTranslatorService,
            IWorkflowMessageService workflowMessageService,
            IVendorService vendorService, IVendorFollowerService vendorFollowerService, IDeviceInfoService deviceInfoService,
            ...
            )
        {
            ...
            _customerTranslatorService = CustomerTranslatorService;
        ...
        #region Utilities
        protected virtual void CustomerTranslationMappings(Product product, ProductModel model)
        {
            var existingCustomerMappings = _customerTranslatorService.GetAllCustomerTranslationByMappings(product);
            if (model.IsTranslate)
            {

                if (existingCustomerMappings.Count > 0)
                {
                    int mappingid = existingCustomerMappings.FirstOrDefault().Id;
                    _customerTranslatorService.UpdateCustomerTranslator(product, model.CustomerId, mappingid, model.IsTranslated);
                }
                else
                {
                    _customerTranslatorService.InsertCustomerTranslationMapping(product, model.CustomerId, model.IsTranslated);
                }
            }
            else if (existingCustomerMappings.Count > 0 && model.IsTranslate == false)
            {
                int mappingid = existingCustomerMappings.FirstOrDefault().EntityId;
                _customerTranslatorService.DeleteCustomerTranslationMapping(product, mappingid);
            }
        }
        ...
        protected virtual void SaveProductWarehouseInventory(Product product, ProductModel model)
        {
            ...
            if (model.ManageInventoryMethodId != (int)ManageInventoryMethod.DontManageStock)
                return;
        ...
        protected virtual void SendPushNotification(Product product, bool previouslyPublished = false)
        {
            if (!previouslyPublished && product.Published && product.VendorId > 0)
            {
                //get vendor followers
                var vendorId = product.VendorId;
                var vendorFollowers = this._vendorFollowerService.GetAllFollowers(vendorId: vendorId);
                foreach (var vendorFollower in vendorFollowers)
                {
                    var customerId = vendorFollower.CustomerId;
                    var customer = _customerService.GetCustomerById(customerId);

                    //grab registeration id
                    var devicesInfo = _deviceInfoService.GetAllDeviceInfo(customerId: customerId);
                    if (devicesInfo.Count > 0)
                    {
                        foreach (var dInfo in devicesInfo)
                        {
                            ...
                            _queuedPushNotificationService.QueueVendorAddProductNotification(vendor, product, customer, registerationId);
                        }
                    }
                }
            }
        ...
        protected virtual void PrepareCustomerTranslationMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            int[] roleid = new int[1];
            var translatorrole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.Translator);
            roleid[0] = translatorrole.Id;
            var allcustomers = _customerService.GetAllCustomers(customerRoleIds: roleid);
            model.AvailableTranslatorCustomer.Add(new SelectListItem
            {
            ...
            if (product != null)
            {
                var existingmapping = _customerTranslatorService.GetCustomerTranslatorByMapping(product);
                if (existingmapping != null)
                {
                    model.CustomerId = existingmapping.CustomerId;
                    model.IsTranslated = existingmapping.IsTranslated;
                    foreach (var customer in allcustomers)
                    {
                        model.AvailableTranslatorCustomer.Add(new SelectListItem
                        {
                            ...
                            Selected = model.CustomerId == existingmapping.CustomerId
                        });

                    }
                    model.IsTranslate = true;
                }
                else
                {
                    foreach (var customer in allcustomers)
        ...
        [HttpPost]
        public IActionResult DeletedProductList(ProductSearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var model = _productModelFactory.PrepareDeleteProductListModel(searchModel);
            return Json(model);
        }
        ...
        public virtual IActionResult Create()
        {
            ...
            PrepareCustomerTranslationMappingModel(model, null, false);

            if (Request.Query["vendor"].Any())
            {
                string vendor = Request.Query["vendor"].ToString();
                model.VendorId = Convert.ToInt32(vendor);
            }
            return View(model);
        }
        ...
        public virtual IActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                product.UpdatedBy = _workContext.CurrentCustomer.Id;
                //customized code
                product.ProductTypeId = 5;
                product.ProductTemplateId = 1;
                //end
                //verify same product is already existing or not
                var searchproductByname = _productService.GetProductByName(model.Name, model.VendorId);
                ...
                var vendor = _vendorService.GetVendorById(product.VendorId);
                if (vendor != null && vendor.CanPublish.HasValue && vendor.CanPublish.Value)
                    _workflowMessageService.SendVendorproductpublishedeNotification(vendor, product, _workContext.WorkingLanguage.Id);

                CustomerTranslationMappings(product, model);
               
                SendPushNotification(product, false);
                ...
                var notificationmessag = $"{_localizationService.GetLocalized(product, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.Added")}";
                _notificationService.SuccessNotification(notificationmessag, name: product.Name);
    ...
    .ToArray();
        ...
        public virtual IActionResult Edit(int id, string selectedTabName = "")
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            PrepareCustomerTranslationMappingModel(model, product, false);
            return View(model);
        }
        ...
        public virtual IActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //if user is admin
                if (_customerService.IsAdmin(_workContext.CurrentCustomer) && product.VendorId > 0)
                {
                    if (model.Published && prevPublished != model.Published)
                    {
                        var vendor = _vendorService.GetVendorById(product.VendorId);
                        if (vendor != null)
                            _workflowMessageService.SendVendorproductpublishedeNotification(vendor, product, _workContext.WorkingLanguage.Id);
                ...
                var newproduct = $"{_localizationService.GetResource("Admin.Catalog.Products.newproduct")}{product.Name}{_localizationService.GetResource("Admin.Catalog.Products.newproductcreated")}";

                var notificationmessag = $"{_localizationService.GetLocalized(product, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.Updated")}";
                _notificationService.SuccessNotification(notificationmessag, name: product.Name);

                var previouslyPublished = model.Published && prevPublished != model.Published;
                //push notification
                SendPushNotification(product, !previouslyPublished);


                if (!continueEditing)
                    return RedirectToAction("List");

                //HttpCookie reqCookies = Request.Cookies["visitedVendor"];
                //if (reqCookies != null)
                //{
                //    var vendor_id = reqCookies["vendor_id"].ToString();
                //    return RedirectToAction("Edit", "Vendor", new { id = vendor_id });
                //}
        ...
        public virtual IActionResult RetrieveProduct(int id)
            {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            _productService.UpdateProduct(product);
            var notificationmessag = $"{_localizationService.GetLocalized(product, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.product.retrieved")}";
            _notificationService.SuccessNotification(notificationmessag, name: product.Name);
            return RedirectToAction("Edit", new { id = product.Id, selectedTabName="" });
        }
        [HttpPost]
        public virtual IActionResult Delete(int id)
        {
            ...
            if (existingCustomerMappings.Count > 0)
            {
                _customerTranslatorService.DeleteCustomerTranslationMapping(product, existingCustomerMappings.FirstOrDefault().Id);
            }
            ...
            var notificationmessag = $"{_localizationService.GetLocalized(product, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.Deleted")}";
            _notificationService.SuccessNotification(notificationmessag, name: product.Name);
        ...
        public virtual IActionResult CopyProduct(ProductModel model)
        {
            ...
            try
            {
                ...
                string Product = copyModel.Name.ToLower().Contains("copy")? copyModel.Name.Substring(0,copyModel.Name.Length - 7): copyModel.Name.ToLower();
                if (originalProduct.Name.ToLower() == Product.ToLower())
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("admin.catalog.products.exists"));
                ...
                //var notificationmessag = $"{copyModel.Name} {_localizationService.GetResource("Admin.Catalog.Products.Deleted")}";
                //activity log
                _customerActivityService.InsertActivity("AddCopyProduct",
                    string.Format(_localizationService.GetResource("ActivityLog.AddNewProduct"), copyModel.Name), newProduct);
                var notificationmessag = $"{_localizationService.GetLocalized(newProduct, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.Products.Added")}";
                _notificationService.SuccessNotification(notificationmessag, name: newProduct.Name);
            ...
            string overrideAltAttribute, string overrideTitleAttribute, int productId)
        {
            ...
            if (_productService.GetProductPicturesByProductId(productId).Any(p => p.PictureId == pictureId))
                ...
                if (_productService.GetProductPicturesByProductId(productId).Count==5)
                return Json(new { Result = false,message="Morethan 5 images not allowed" });
        ...
        public virtual IActionResult ProductTagDelete(int id)
        {
            ...
            var notificationmessag = $"{_localizationService.GetLocalized(tag, x => x.Name, _workContext.WorkingLanguage.Id)} {_localizationService.GetResource("Admin.Catalog.ProductTags.Deleted")}";
            _notificationService.SuccessNotification(notificationmessag, name: tag.Name);
        ...
        public virtual IActionResult ImportExcel(IFormFile importexcelfile)
        {
            ...
            catch (ArgumentNullException ex)
            {
                ...
                _notificationService.ErrorNotification(msg+"Name,Description and Price is mandatory for each poduct please reiew the import file");
                return RedirectToAction("List");
            }
            catch (Exception ex)
            {
                _notificationService.ErrorNotification("Invalid Template used");
                return RedirectToAction("List");
            }
        ...
        [HttpPost]
        public IActionResult ImportExcelOldSystem(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            try
            {
                if (importexcelfile != null && importexcelfile.Length > 0)
                {
                    _importManager.ImportProductsOldSystemFromXlsx(importexcelfile.OpenReadStream());
                }
                else
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
        ...
        [HttpPost]
        public IActionResult ImportOldSystemPictures(IFormFile importexcelfile)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            try
            {
                if (importexcelfile != null && importexcelfile.Length > 0)
                {
                    _importManager.ImportOldSystemPicturesFromXlsx(importexcelfile.OpenReadStream());
                }
                else
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Common.UploadFile"));
        ...
        public IActionResult BulkEdit()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            //prepare model
            var model = _productModelFactory.PrepareBulkEditProductModel(new BulkEditSearchModel());
            return View(model);
        }
        [HttpPost]
        public IActionResult BulkEditSelect(BulkEditSearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var model = _productModelFactory.PrepareBulkEditSelectProductModel(searchModel);
        ...
        {
            List<BulkEditProductModel> models = new List<BulkEditProductModel>();
