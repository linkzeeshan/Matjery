...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public ProductModelFactory(CatalogSettings catalogSettings,
            ...
            VendorSettings vendorSettings, ICustomerTranslatorService customerTranslatorService, IVendorService vendorService)
        {
        ...
        protected virtual CopyProductModel PrepareCopyProductModel(CopyProductModel model, Product product)
        {
            ...
            model.Id = product.Id;
            var productName = _workContext.WorkingLanguage.Id == 1 ? _localizationService.GetLocalized(product, entity => entity.Name, _workContext.WorkingLanguage.Id) : product.Name;
            model.Name = string.Format(_localizationService.GetResource("Admin.Catalog.Products.Copy.Name.New"), productName);
        ...
        public virtual ProductSearchModel PrepareProductSearchModel(ProductSearchModel searchModel)
        {
            ...
            _baseAdminModelFactory.PrepareCategories(searchModel.AvailableCategories,languageid:_workContext.WorkingLanguage.Id);
        ...
        public virtual ProductListModel PrepareProductListModel(ProductSearchModel searchModel)
        {
            ...
            if (_customerService.IsTranslator(_workContext.CurrentCustomer))
            {
                ...
                products = _customerTranslatorService.GetAllProductCustomerTranslationListByMappings(Prodcut, _workContext.CurrentCustomer.Id,
                    searchModel.SearchProductName, _workContext.WorkingLanguage.Id, showNotTranslatedOnly: true);

            }
            else
            {
                //get products
                products = _productService.SearchProducts(showHidden: true,
            ...
            var model = new ProductListModel().PrepareToGrid(searchModel, products, () =>
            {
                return products.Select(product =>
                {
                    ...
                    productModel.Name = _localizationService.GetLocalized(product, p => p.Name);
                    ...
                    productModel.VendorName = product.VendorId != 0 ? _vendorService.GetVendorById(product.VendorId)?.Name : null;
                    ...
                    productModel.CreatedOn = _dateTimeHelper.ConvertToUserTime(product.CreatedOnUtc, DateTimeKind.Utc).ToString();
                    return productModel;
                });
            });
            return model;
        ...
        /// <returns></returns>
        public virtual ProductListModel PrepareDeleteProductListModel(ProductSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            var products = _productService.deletedProduct(pageIndex: searchModel.Page - 1,pageSize: searchModel.PageSize, searchModel.SearchVendorId);
            ...
            var model = new ProductListModel().PrepareToGrid(searchModel, products, () =>
            {
                return products.Select(product =>
                {
                    ...
                    var defaultProductPicture = _pictureService.GetPicturesByProductId(product.Id, 1).FirstOrDefault();
                    productModel.PictureThumbnailUrl = _pictureService.GetPictureUrl(ref defaultProductPicture, 75);
                    productModel.Name =_localizationService.GetLocalized(product,c => c.Name, _workContext.WorkingLanguage.Id);
                    //product type
                    productModel.ProductTypeName = _localizationService.GetLocalizedEnum(product.ProductType);
        ...
        public virtual ProductModel PrepareProductModel(ProductModel model, Product product, bool excludeProperties = false)
        {
            ...
            _baseAdminModelFactory.PrepareCategories(model.AvailableCategories, false, languageid:_workContext.WorkingLanguage.Id);
            ...
            //model.PicturesCount = _productService.GetProductPicturesByProductId(product.Id).Count;
            return model;
        }
        ...
        /// <param name="searchModel">Required product search model to add to the product</param>
        /// <returns>Required product list model to add to the product</returns>
        public virtual BulkEditListModel PrepareBulkEditSelectProductModel(BulkEditSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            var products = _productService.SearchProducts(showHidden: true,
                ...
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);
            //prepare grid model
            var model = new BulkEditListModel().PrepareToGrid(searchModel, products, () =>
            {
                if (vendorId != 0)
                {
                    
                    return products.Where(x => x.VendorId == vendorId).Select(product =>
                    { 

                        var bulkEditproductModel = product.ToModel<BulkEditProductModel>();
                        var vendor = _vendorService.GetVendorById(product.VendorId);
                        bulkEditproductModel.Name = _localizationService.GetLocalized(product, x => x.Name);
                        bulkEditproductModel.ManageInventoryMethod = _localizationService.GetLocalizedEnum(product.ManageInventoryMethod, _workContext.WorkingLanguage.Id);
                        bulkEditproductModel.VendorName = product.VendorId != 0 ? _localizationService.GetLocalized(vendor, x => x.Name) : null;
                        return bulkEditproductModel;
                    });
                }
                else
                {
        ...
        public virtual List<BulkEditProductModel> PrepareBulkEditPProductModel(BulkEditSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            var products = _productService.SearchProducts(showHidden: true,
                ...
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);
            //prepare grid model
                var model =  products.Select(product =>
                {
                    var bulkEditproductModel = product.ToModel<BulkEditProductModel>();
                    bulkEditproductModel.ManageInventoryMethod = _localizationService.GetLocalizedEnum(product.ManageInventoryMethod, _workContext.WorkingLanguage.Id);
        ...
        /// Prepare product model
        /// </summary>
        /// <param name="model">Product List model</param>
        /// <returns>Product list model</returns>
        public virtual BulkEditSearchModel PrepareBulkEditProductModel(BulkEditSearchModel model)
        {
            //prepare available categories
