...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class CustomerModelFactory : ICustomerModelFactory
    {
        ...
        protected virtual void PrepareAddressModel(AddressModel model, Address address, bool showCountry = false)
        {
            if (model == null)
                throw new ArgumentNullException(nameof(model));
            ...
            model.CountryEnabled = _addressSettings.CountryEnabled= showCountry;
            model.CountryRequired = _addressSettings.CountryEnabled= showCountry; //country is required when enabled
        ...
        public virtual CustomerSearchModel PrepareCustomerSearchModel(CustomerSearchModel searchModel)
        {
            ...
            if (_customerSettings.CountryEnabled)
            {
                ...
                if (_customerSettings.StateProvinceEnabled)
                    _baseAdminModelFactory.PrepareStatesAndProvinces(searchModel.AvailableStates, 81);
            }
         

            //country
            searchModel.AvailableCountries.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Address.SelectCountry"), Value = "0" });
            foreach (var c in _countryService.GetAllCountries(showHidden: true))
            {
                searchModel.AvailableCountries.Add(new SelectListItem
                {
                    Text = _localizationService.GetLocalized(c,cc => cc.Name),
                    Value = c.Id.ToString()
                    //Selected = c.Id == model.CountryId
                });
            }
            //states
            var states = _stateProvinceService.GetStateProvincesByCountryId(81).ToList();
            if (states.Any())
            {
                searchModel.AvailableStates.Add(new SelectListItem { Text = _localizationService.GetResource("Admin.Address.SelectState"), Value = "0" });

                foreach (var s in states)
                {
                    searchModel.AvailableStates.Add(new SelectListItem { Text = _localizationService.GetLocalized(s,n => n.Name), Value = s.Id.ToString(), Selected = (s.Id == searchModel.StateProvinceId) });
                }
            }
            //prepare page parameters
            searchModel.SetGridPageSize();
        ...
        public virtual CustomerListModel PrepareDeletedCustomerListModel(CustomerSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            IPagedList<Customer> customerslist = null;
            var customers = _customerService.Deletedcustomers(pageIndex: searchModel.Page - 1,
                     pageSize: searchModel.PageSize);

            //verify same customer is already registerd or  not
            customers = CustomerList(customers, searchModel.Page - 1, searchModel.PageSize);
            ...
            var model = new CustomerListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    var total = _blackPointService.SearchBlackPoints(vendorOrCustomerId: customer.Id
                    , blackPointType: (int)BlackPointTypeEnum.Customer, pageSize: 1, blackPointStatus: (int)BlackPointStatusEnum.Approved).TotalCount;
                    customerModel.TotalBlackPoints = total;
        ...
        public virtual CustomerListModel PrepareCustomerListModel(CustomerSearchModel searchModel)
        {
            ...
            if (searchModel.SearchForVendors)
            {
                int[] VendorJust = { 5 };
                searchModel.SelectedCustomerRoleIds = VendorJust;
            }
            var orderBy = CustomerSortingEnum.CreatedOnDesc;
            ...
            //        switch (st.Field)
            //        {
            //            case "Email": orderBy = CustomerSortingEnum.EmailAsc; break;
            //            case "Username":
            //                orderBy = CustomerSortingEnum.UserNameAsc;
            //                break;
            //            case "FullName":
            //                orderBy = CustomerSortingEnum.NameAsc;
            //                break;
            //            case "CreatedOn": orderBy = CustomerSortingEnum.CreatedOnAsc; break;
            //        }
            ...
            //        switch (st.Field)
            //        {
            //            case "Email": orderBy = CustomerSortingEnum.EmailDesc; break;
            //            case "Username":
            //                orderBy = CustomerSortingEnum.UserNameDesc;
            //                break;
            //            case "FullName":
            //                orderBy = CustomerSortingEnum.NameDesc;
            //                break;

            //            case "CreatedOn": orderBy = CustomerSortingEnum.CreatedOnDesc; break;
            //        }
            //    }
            //}
            //get customers
        ...
        public virtual CustomerModel PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties = false, bool showCountry = false)
        {
            if (customer != null)
            {
                //fill in model values from the entity
                ...
                model.CreatedBy = customer.CreatedBy;
                model.UpdatedBy = customer.UpdatedBy;
                ...
                if (!excludeProperties)
                {
                    ...
                    {
                        var countryNationalityId = _genericAttributeService.GetAttribute<int>(customer, NopCustomerDefaults.NationalityCountryId);
                        if (countryNationalityId > 0)
                        {
                            model.CountryId = countryNationalityId;
                        }
                    }
                    ...
                    model.HasAcceptedTermsAndConditions = _genericAttributeService.GetAttribute<bool>(customer,NopCustomerDefaults.HasAcceptedTermsAndConditions);
            ...
            model.CountyEnabled = _customerSettings.CountyEnabled = showCountry;
            model.StateProvinceEnabled = _customerSettings.StateProvinceEnabled = showCountry;
            model.PhoneEnabled = _customerSettings.PhoneEnabled;
            model.FaxEnabled = _customerSettings.FaxEnabled;
            if (customer != null && customer.CreatedBy.HasValue)
                model.CreatedByStr = _customerService.GetCustomerFullName( _customerService.GetCustomerById(customer.CreatedBy.Value));

            if (customer != null && customer.UpdatedBy.HasValue)
                model.UpdatedByStr = _customerService.GetCustomerFullName(_customerService.GetCustomerById(customer.UpdatedBy.Value));
        ...
        /// <summary>
        /// Prepare  customer order list model
        /// </summary>
        /// <param name="customerId">Customer Id</param>
        /// <returns>Customer order list model</returns>
        public virtual List<Order> PrepareCustomerOrderListModel(int customerId, Customer customer)
        {
            if (customerId == 0)
            ...
            //get customer orders
            var orders = _orderService.GetOrderByCustomOrderById(customerId);
        ...
        public bool CompareDeleteCustomerWithCustomer(Customer customer)
        {
            if (customer.Email.Contains("deleted_"))
            {
                var changeEmailFormat = CommonHelper.CustomerEmailFormat(customer.Email);
                var deleteCustomerExist = _customerService.GetCustomerByEmail(changeEmailFormat);
                if (deleteCustomerExist != null)
                    return true;
            }
            return false;
        }
        public virtual IPagedList<Customer> CustomerList(IPagedList<Customer> customers, int page = 0, int pagesize = int.MaxValue)
        {
            ...
            foreach (var customer in customers)
            {
                if (customer.Email.Contains("deleted_"))
                {
                    ...
                    var email = CommonHelper.CustomerEmailFormat(customer.Email);
                    var deletedUserRegisterExist = _customerService.GetCustomerByEmail(email);
            ...
            customers = new PagedList<Customer>(customerslist, page, pagesize);
            return customers;
        }
        #endregion
    }
