...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public ProductModelFactory(CaptchaSettings captchaSettings,
            ...
            IVendorRatingService vendorRatingServicey,
            VendorSettings vendorSettings,
            IShoppingCartService shoppingCartService
            )
        {
            _shoppingCartService = shoppingCartService;
            _vendorRatingService = vendorRatingServicey;
            ...
            if (productReview != null)
            {
                ...
                if (!_customerService.IsGuest(_workContext.CurrentCustomer))
                {
                    ...
                    if (totalCustomerOrders > 0)
                    {
                        ...
                        if (totalCustomerOrders > totalCustomerReviews)
                            productReview.CanCurrentCustomerReview = true;
        ...
        protected virtual void PrepareSimpleProductOverviewPriceModel(Product product, ProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            //var currentProductInWishList = _workContext.CurrentCustomer.ShoppingCartItems
            //    .Where(sci => sci.ShoppingCartType == ShoppingCartType.Wishlist && product.Id == sci.ProductId)
        ...
        protected virtual ProductDetailsModel.AddToCartModel PrepareProductAddToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            var currentProductInWishList = _workContext.CurrentCustomer.ShoppingCartItems
             .Where(sci => sci.ShoppingCartType == ShoppingCartType.Wishlist && product.Id == sci.ProductId && sci.StoreId==_storeContext.CurrentStore.Id)
             .FirstOrDefault();
            ...
            //quantity
            var currentProductInCartList = _workContext.CurrentCustomer.ShoppingCartItems
            .Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart && product.Id == sci.ProductId).FirstOrDefault();

            model.EnteredQuantity = updatecartitem != null ? updatecartitem.Quantity : product.OrderMinimumQuantity;
            if (currentProductInCartList != null)
            {
                model.ExistInCart = true;
                model.CartQuantity = currentProductInCartList.Quantity;
            }
            else
            {
                model.ExistInCart = false;
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
           ...
           _urlRecordService.GetSeName(product, _workContext.WorkingLanguage.Id));
            ...
            var model = new ProductDetailsModel
            {
                Id = product.Id,
                Name = _localizationService.GetLocalized(product, x => x.Name),
                ShortDescription =CommonHelper.StripHTML(_localizationService.GetLocalized(product, x => x.ShortDescription)),
                FullDescription = CommonHelper.StripHTML(_localizationService.GetLocalized(product, x => x.FullDescription)),
            ...
            var currentProductInWishList = shoppingCartItems
                 .Where(sci => product.Id == sci.ProductId)
                 .LimitPerStore(_storeContext.CurrentStore.Id)
                 .FirstOrDefault();
            model.IsAddedToWishlist = currentProductInWishList != null ? true : false;
            ...
            if (_vendorSettings.ShowVendorOnProductDetailsPage)
            {
                ...
                if (vendor != null && !vendor.Deleted && vendor.Active)
                {
                    ...
                    string vendorUrl = string.Format("{0}v/{1}", _webHelper.GetStoreLocation(), _urlRecordService.GetSeName(vendor,_workContext.WorkingLanguage.Id));
                    ...
                    {
                        model.VendorModel.FoundationAprovalStatusText =_localizationService.GetLocalizedEnum(vendor.FoundationAprovalStatus, _workContext.WorkingLanguage.Id);
                    }
                    model.VendorModel.ShortDescription = model.VendorModel.Description.TruncateToShort();
                    foreach (Language language in _languageService.GetAllLanguages())
                    {
                        var localeModel = new
                        {
                            ...
                            Description = CommonHelper.StripHTML(_localizationService.GetLocalized(vendor,x => x.Description, language.Id)),
                            ShortDescription = CommonHelper.StripHTML(_localizationService.GetLocalized(vendor,x => x.Description, language.Id)).TruncateToShort(),
                            LicensedBy = _localizationService.GetLocalized(vendor,v => v.LicensedBy, language.Id)
                        };
                        model.VendorModel.CustomProperties.Add(language.UniqueSeoCode, localeModel);
                    }
                    ...
                    if (vendor.PictureId > 0)
                    {
                        ...
                        model.VendorModel.FullSizeImageUrl = _pictureService.GetPictureUrl(picture.Id, defaultPictureType: PictureType.VendorOrEntity);
                        model.VendorModel.ImageUrl = _pictureService.GetPictureUrl(picture.Id, pictureSize, defaultPictureType: PictureType.VendorOrEntity);
                    }
                    model.VendorModel.WaterMarkImageUrl = "";// _miscWatermarkPictureService.GetPictureUrlWithWatermark(picture, pictureSize, defaultPictureType: PictureType.VendorOrEntity);
                    model.VendorModel.TotalProducts = _productService.GetNumberOfProductsByVendorId(vendor.Id);
                    ...
                    if (vendorReviews.TotalCount != 0)
                    {
                        ...
                        model.VendorModel.VendorReview.TotalRating = percentage * 5 / 100;
                    }
                    //followers
                    ...
                    {
                        var vendorfollowerstModel = new VendorBriefInfoModel.VendorFollowerModel.FollowersModel();
                        ...
                        model.VendorModel.VendorFollower.Followers.Add(vendorfollowerstModel);
                    }
                    //license
                    model.VendorModel.LicenseNo = vendor.LicenseNo;
                    if (vendor.LicenseCopyId > 0)
                    {
                        ...
                        if (licenseCopyDownload != null)
                            model.VendorModel.HasUploadedLicense = true;
                    }

                    model.VendorModel.VendorFollower.TotalFollowers = vendorFollowers.TotalCount;
