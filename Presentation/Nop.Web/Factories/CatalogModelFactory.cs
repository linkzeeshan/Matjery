...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual VendorModel PrepareVendorModel(Vendor vendor, CatalogPagingFilteringModel command)
        {
            ...
            model.PictureModel = _staticCacheManager.Get(pictureCacheKey, () =>
            {
                ...
                var pictureModel = new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                    ImageUrl = _pictureService.GetPictureUrl(ref picture, pictureSize),
                    Title = string.Format("{0}", _localizationService.GetResource("Media.Vendor.ImageLinkTitleFormat")), //vendorModel.Name
                    AlternateText = string.Format("{0}", _localizationService.GetResource("Media.Vendor.ImageAlternateTextFormat"))
                };

                return pictureModel;
            });

            if (vendor.FoundationTypeId == (int)VendorRegisterationType.Foundation)
            {
                model.SupportedByFoundationId = model.Id;
                vendor.SupportedByFoundationId = model.Id;
            }

            model.SupportedByFoundationId = vendor.SupportedByFoundationId;

            //supportedByFoundationId
            if (model.SupportedByFoundationId.HasValue)
            {
                var supportedByFoundation = _vendorService.GetVendorById(model.SupportedByFoundationId.HasValue ? model.SupportedByFoundationId.Value : 0);
                if (supportedByFoundation != null)
                {
                    model.SupportedByFoundationDescription = _localizationService.GetLocalized(supportedByFoundation, x => x.Description);
                    var supportedByFoundationpictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorPictureModelKey,
                                                                supportedByFoundation.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);

                    model.SupportedByFoundationPictureModel = _staticCacheManager.Get(supportedByFoundationpictureCacheKey, () =>
                    {
                        var picture = _pictureService.GetPictureById(supportedByFoundation.PictureId);
                        var pictureModel = new PictureModel
                        {
                            FullSizeImageUrl = _pictureService.GetPictureUrl(picture.Id, defaultPictureType: PictureType.VendorOrEntity),
                            ImageUrl = _pictureService.GetPictureUrl(picture.Id, pictureSize, defaultPictureType: PictureType.VendorOrEntity),
                ...
                var modelSupBy = new VendorModel
                {
                    Id = supportedByFoundation.Id,
                    SeName = _urlRecordService.GetSeName(supportedByFoundation)

                };
                model.SupportedBySeName = modelSupBy.SeName;
            }
            if (string.IsNullOrEmpty(vendor.PageSizeOptions))
            {
            ...
            PreparePageSizeOptions(model.PagingFilteringContext, command,
                ...
                vendor.PageSize);
            //only the vendor who has registration id = merchant and foundation type cannot approved or reject will see the products
            //products
            if ((vendor.RegisterationTypeId == 1) || (vendor.RegisterationTypeId == 2 && vendor.FoundationTypeId == 1))
            {
                //products
                if (vendor.Active)
                {
            ...
            else if (vendor.RegisterationTypeId == 2 && vendor.FoundationTypeId == 2)
            {
                var vendors = _vendorService.GetAllVendorsSupportedByFoundation(Convert.ToInt32(vendor.SupportedByFoundationId), 2, 0, int.MaxValue);
                if (vendors.Any())
                {
                    foreach (var vendorr in vendors)
                    {
                        var vendorView = new VendorModel
                        {
                            Id = vendorr.Id,
                            Name = _localizationService.GetLocalized(vendorr, x => x.Name),
                            Description = _localizationService.GetLocalized(vendorr, x => x.Description),
                            MetaKeywords = _localizationService.GetLocalized(vendorr, x => x.MetaKeywords),
                            MetaDescription = _localizationService.GetLocalized(vendorr, x => x.MetaDescription),
                            MetaTitle = _localizationService.GetLocalized(vendorr, x => x.MetaTitle),
                            SeName = _urlRecordService.GetSeName(vendorr),
                            AllowCustomersToContactVendors = _vendorSettings.AllowCustomersToContactVendors
                        };

                        //prepare picture model
                        int pictureSize_ = _mediaSettings.VendorThumbPictureSize;
                        var pictureCacheKey_ = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorPictureModelKey,
                        vendorr.Id, pictureSize_, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                        vendorView.PictureModel = _staticCacheManager.Get(pictureCacheKey_, () =>
                        {
                            var picture = _pictureService.GetPictureById(vendorr.PictureId);
                            var pictureModel = new PictureModel
                            {
                                FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture, defaultPictureType: PictureType.VendorOrEntity),
                                ImageUrl = _pictureService.GetPictureUrl(ref picture, pictureSize_, defaultPictureType: PictureType.VendorOrEntity),
                        ...
                        model.SearchedVendors.Add(vendorView);
                    }

                }
            }
            //license 
            if (!string.IsNullOrEmpty(vendor.LicenseNo))
            {
                model.LicenseNo = vendor.LicenseNo;
                if (!string.IsNullOrEmpty(vendor.LicensedBy))
                    model.LicensedBy = _localizationService.GetLocalized(vendor, b => b.LicensedBy);
                if (vendor.LicenseCopyId > 0)
                {
                    ...
                    if (licenseCopyDownload != null)
                        model.HasUploadedLicense = true;
                }
            }
            //social media
            if (!string.IsNullOrEmpty(vendor.WhatsApp))
                model.WhatsApp = vendor.WhatsApp;

            if (!string.IsNullOrEmpty(vendor.Facebook))
                model.Facebook = vendor.Facebook;

            if (!string.IsNullOrEmpty(vendor.Twitter))
                model.Twitter = vendor.Twitter;

            if (!string.IsNullOrEmpty(vendor.Instagram))
                model.Instagram = vendor.Instagram;

            if (!string.IsNullOrEmpty(vendor.Googleplus))
                model.Googleplus = vendor.Googleplus;

            if (!string.IsNullOrEmpty(vendor.BBM))
        ...
        public virtual VendorListModel PrepareVendorAllModels(CatalogPagingFilteringModel command, bool displayEntities = false)
        {
            ...
            var vendors = _vendorService.GetAllEntitiesMod(
                vendorRegisterationType: displayEntities ? VendorRegisterationType.Foundation : VendorRegisterationType.Merchant,
                  pageSize: command.PageSize,
                  pageIndex: command.PageNumber - 1, customer: _workContext.CurrentCustomer);
            ...
            foreach (var vendor in vendors)
            {
                ...
                model.VendorModel.Add(vendorModel);
            }

            return model;
        }

        public virtual List<VendorModel> PrepareVendor(bool? displayEntities = false, int Pagesize = 10)
        {

            var model = new List<VendorModel>();

            var merchants = Convert.ToBoolean(displayEntities) ? _vendorService.GetAllEntitiesMod(
            vendorRegisterationType: Convert.ToBoolean(displayEntities) ? VendorRegisterationType.Foundation : VendorRegisterationType.Merchant,
              ...
              vendorRegisterationType: Convert.ToBoolean(displayEntities) ? VendorRegisterationType.Foundation : VendorRegisterationType.Merchant,
              pageIndex: 0, pageSize: Pagesize, showNotActive: false);
            ...
            foreach (var merchant in merchants)
            {
                var vendorModel = new VendorModel
                {
                    ...
                    SeName = _urlRecordService.GetSeName(merchant)
                    //AllowCustomersToContactVendors = _vendorSettings.AllowCustomersToContactVendors
                };
                //prepare picture model
                int pictureSize = _mediaSettings.VendorThumbPictureSize;
                var pictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorPictureModelKey,
                merchant.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);

                vendorModel.PictureModel = _staticCacheManager.Get(pictureCacheKey, () =>
                {
                    var picture = _pictureService.GetPictureById(merchant.PictureId);
                    var pictureModel = new PictureModel
                    {
        ...
        public virtual VendorNavigationModel PrepareVendorNavigationModel()
        {
            ...
            var cachedModel = _staticCacheManager.Get(cacheKey, () =>
            {
                var vendors = _vendorService.GetAllVendors(showHidden: false, pageSize: _vendorSettings.VendorsBlockItemsToDisplay);
        ...
        #region Searching
        public virtual SearchModel PrepareNewSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            if (model == null)
            ...
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.SEARCH_CATEGORIES_MODEL_KEY,
            ...
            var categories = _staticCacheManager.Get(cacheKey, () =>
            {
                ...
                foreach (var c in allCategories)
                {
                    ...
                    var breadcrumb = _categoryService.GetCategoryBreadCrumb(c, allCategories, true);
            ...
            {
                var stype = _httpContextAccessor.HttpContext.Request.Query.ContainsKey("search_type");
                string searchtype = stype.ToString();
                if (model.searchType == "P" || model.searchType == "p" || searchtype == "P")
                {
                    ...
                    model.adv = true;   //for advanced search
                    if (model.adv)
                    {
                        //advanced search
                    ...
                    _commonSettings.UseStoredProceduresIfSupported = false;
                    //products
                    //products = _productService.SearchProductsMod(
                    //    categoryIds: categoryIds,
                ...
                else if (model.searchType == "S")
                {
                    searchTerms = model.qs;
                    if (searchTerms == null)
                        searchTerms = "";
                    searchTerms = searchTerms.Trim();
                    var vendors = _vendorService.GetAllVendors(showHidden: false, name: searchTerms, pageSize: int.MaxValue);
                    if (vendors.Any())
                    {
                        foreach (var vendor in vendors)
                        {
                            ...
                            var pictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.VendorPictureModelKey, vendor.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
