    ...
    public partial class AddressModelFactory : IAddressModelFactory
    {
            ...
            string overrideAttributesXml = "")
        {
            ...
            if (!excludeProperties && address != null)
            {
                ...
                model.IsDefault = address.IsDefault == true ?Convert.ToBoolean(address.IsDefault) : false;
            }
            var constantService = EngineContext.Current.Resolve<IConstantService>();
            IPagedList<Constant> constants = constantService.GetAllConstants();
            ...
            if (address == null && prePopulateWithCustomerFields)
            {
                ...
                if(address!=null)
                {
                    ...
                    {
                        var area = constants.FirstOrDefault(a => a.GroupName == "AR" && a.Code == address.Area);
                        if (area != null)
                        {
                            model.AreaName = workContext.WorkingLanguage.Rtl ? area.ArabicName : area.EnglishName;
                        }
                    }
            ...
            if (addressSettings.CountryEnabled && loadCountries != null)
            {
                ...
                {
                    model.CountryId = 81;// countries[0].Id;
                }
                else
                {
                ...
                //{
                //    areas.AddRange(constants.Where(a => a.GroupName == "AR" && a.CityId == city.Code));
                //}
                foreach (Constant area in areas)
                {
                    model.AvailableAreas.Add(new SelectListItem
                    {
                        Text = workContext.WorkingLanguage.Rtl ? area.ArabicName : area.EnglishName,
                        Value = area.Code,
