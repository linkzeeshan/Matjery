...
namespace Nop.Web.Controllers
{
    ...
    public partial class ShoppingCartController : BasePublicController
    {
            ...
            ShoppingCartSettings shoppingCartSettings)
        {
            ...
            //canot inject it to constructor as it requires by nop plugins
        ...
        protected virtual void SaveItem(ShoppingCartItem updatecartitem, List<string> addToCartWarnings, Product product,
           ...
           DateTime? rentalEndDate, int quantity, bool shouldDelete,int shoppingCartTypeId)
        {
            if (updatecartitem == null)
            {
                if (shouldDelete && (ShoppingCartType)shoppingCartTypeId == ShoppingCartType.Wishlist)
                    quantity = -1;
                //add to the cart
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, IFormCollection form, bool shouldDelete)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
            {
            ...
            SaveItem(updatecartitem, addToCartWarnings, product, cartType, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, quantity, shouldDelete,shoppingCartTypeId);
        ...
        public virtual IActionResult Cart()
        {
            ...
            if (TempData["cartmodel"] != null)
             tempmodel = JsonConvert.DeserializeObject<ShoppingCartModel>(TempData["cartmodel"].ToString());

            model = tempmodel.Items.Count() == 0 ?  _shoppingCartModelFactory.PrepareShoppingCartModel(model, cart) : tempmodel;
            TempData["cartmodel"] = null;
            return View(model);
        }
        ...
        public virtual IActionResult ApplyDiscountCoupon(string discountcouponcode, IFormCollection form)
        {
            ...
            if (!string.IsNullOrWhiteSpace(discountcouponcode))
            {
                ...
                var discounts = _discountService.GetAllDiscounts(DiscountType.AssignedToOrderTotal, showHidden: true)
            ...
            model = _shoppingCartModelFactory.PrepareShoppingCartModel(model, cart);
            TempData["cartmodel"] = JsonConvert.SerializeObject(model);
